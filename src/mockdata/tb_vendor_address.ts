...
export const createVendorAddress = (data: Omit<VendorAddress, 'id' | 'created_at' | 'created_by_id' | 'updated_at' | 'updated_by_id'>): VendorAddress => {
...
export const getVendorAddressesByVendor = (vendorId: string): VendorAddress[] => {
  return vendorAddresses.filter(address => address.vendor_id === vendorId && !address.deleted_at);
};

export const getVendorAddressesByType = (addressType: 'billing' | 'shipping' | 'contact' | 'other'): VendorAddress[] => {
  return vendorAddresses.filter(address => address.address_type === addressType && !address.deleted_at);
};

export const getVendorAddressesByVendorAndType = (vendorId: string, addressType: 'billing' | 'shipping' | 'contact' | 'other'): VendorAddress[] => {
...
export const getActiveVendorAddresses = (): VendorAddress[] => {
  return vendorAddresses.filter(address => address.is_active && !address.deleted_at);
};
...
export const getVendorAddressesByCreator = (createdById: string): VendorAddress[] => {
  return vendorAddresses.filter(address => address.created_by_id === createdById && !address.deleted_at);
};

export const getVendorAddressesByDateRange = (startDate: string, endDate: string): VendorAddress[] => {
  return vendorAddresses.filter(address => {
...
export const getVendorAddressesWithNote = (): VendorAddress[] => {
  return vendorAddresses.filter(address => address.note !== null && !address.deleted_at);
};
...
export const updateVendorAddress = (id: string, data: Partial<Omit<VendorAddress, 'id' | 'created_at' | 'created_by_id'>>): VendorAddress | null => {
  const index = vendorAddresses.findIndex(address => address.id === id && !address.deleted_at);
...
export const updateVendorAddressStatus = (id: string, isActive: boolean): VendorAddress | null => {
  return updateVendorAddress(id, { is_active: isActive });
};

// UPDATE - อัปเดต VendorAddress type
export const updateVendorAddressType = (id: string, addressType: 'billing' | 'shipping' | 'contact' | 'other'): VendorAddress | null => {
  return updateVendorAddress(id, { address_type: addressType });
};

// UPDATE - อัปเดต VendorAddress data
export const updateVendorAddressData = (id: string, data: any): VendorAddress | null => {
  return updateVendorAddress(id, { data });
};

// UPDATE - อัปเดต VendorAddress description
export const updateVendorAddressDescription = (id: string, description: string): VendorAddress | null => {
  return updateVendorAddress(id, { description });
};

// UPDATE - อัปเดต VendorAddress note
export const updateVendorAddressNote = (id: string, note: string): VendorAddress | null => {
  return updateVendorAddress(id, { note });
};

// UPDATE - อัปเดต VendorAddress info
export const updateVendorAddressInfo = (id: string, info: any): VendorAddress | null => {
  return updateVendorAddress(id, { info });
};

// UPDATE - อัปเดต VendorAddress dimension
export const updateVendorAddressDimension = (id: string, dimension: any): VendorAddress | null => {
  return updateVendorAddress(id, { dimension });
};

// DELETE - Soft delete VendorAddress
export const softDeleteVendorAddress = (id: string, deletedById: string): VendorAddress | null => {
...
export const deleteVendorAddressesByVendor = (vendorId: string, deletedById: string): boolean => {
...
export const deleteVendorAddressesByType = (addressType: 'billing' | 'shipping' | 'contact' | 'other', deletedById: string): boolean => {
...
export const searchVendorAddresses = (criteria: {
  ...
  return vendorAddresses.filter(address => {
    ...
    if (criteria.address_type && address.address_type !== criteria.address_type) {
      return false;
    }
    
    if (criteria.is_active !== undefined && address.is_active !== criteria.is_active) {
      return false;
    }
    ...
    if (criteria.created_by_id && address.created_by_id !== criteria.created_by_id) {
      return false;
    }
...
export const getVendorAddressCountByVendor = (vendorId: string): number => {
  return vendorAddresses.filter(address => address.vendor_id === vendorId && !address.deleted_at).length;
};

export const getVendorAddressCountByType = (addressType: 'billing' | 'shipping' | 'contact' | 'other'): number => {
  return vendorAddresses.filter(address => address.address_type === addressType && !address.deleted_at).length;
};

export const getActiveVendorAddressCount = (): number => {
  return vendorAddresses.filter(address => address.is_active && !address.deleted_at).length;
};

export const isVendorAddressExists = (id: string): boolean => {
  return vendorAddresses.some(address => address.id === id && !address.deleted_at);
};

export const hasVendorAddressesByVendor = (vendorId: string): boolean => {
  return vendorAddresses.some(address => address.vendor_id === vendorId && !address.deleted_at);
};

export const hasVendorAddressesByType = (addressType: 'billing' | 'shipping' | 'contact' | 'other'): boolean => {
  return vendorAddresses.some(address => address.address_type === addressType && !address.deleted_at);
};

export const hasVendorAddressesWithNote = (): boolean => {
  return vendorAddresses.some(address => address.note !== null && !address.deleted_at);
};

...
export const getStockOutsByStatus = (
...
export const getStockOutsByWorkflow = (workflowId: string): StockOut[] => {
  return stockOuts.filter(
    (so) => so.workflow_id === workflowId && !so.deleted_at
  );
};

export const getStockOutsByCreator = (createdById: string): StockOut[] => {
  return stockOuts.filter(
    (so) => so.created_by_id === createdById && !so.deleted_at
  );
};

export const getStockOutsByDateRange = (
  startDate: string,
  endDate: string
): StockOut[] => {
...
export const updateStockOut = (
  id: string,
  data: Partial<Omit<StockOut, "id" | "created_at" | "created_by_id">>
): StockOut | null => {
...
export const updateStockOutStatus = (
  id: string,
  status: StockOut["doc_status"]
): StockOut | null => {
...
export const updateStockOutWorkflow = (
  id: string,
  workflowId: string,
  workflowName: string
): StockOut | null => {
...
export const updateStockOutNote = (
  id: string,
  note: string
): StockOut | null => {
...
export const updateStockOutDimension = (
  id: string,
  dimension: any
): StockOut | null => {
...
export const softDeleteStockOut = (
  id: string,
  deletedById: string
): StockOut | null => {
...
export const searchStockOuts = (criteria: {
  ...
  return stockOuts.filter((so) => {
    ...
    if (criteria.workflowId && so.workflow_id !== criteria.workflowId)
      return false;
    if (criteria.createdById && so.created_by_id !== criteria.createdById)
      return false;
    ...
    if (
      ...
      !so.description.toLowerCase().includes(criteria.description.toLowerCase())
    )
      return false;
    if (
      criteria.note &&
      !so.note.toLowerCase().includes(criteria.note.toLowerCase())
    )
      return false;
...
export const getStockOutCountByStatus = (
  status: StockOut["doc_status"]
): number => {
  return stockOuts.filter((so) => so.doc_status === status && !so.deleted_at)
    .length;
...
export const getNextStockOutNo = (): string => {
  ...
  const yearStockOuts = stockOuts.filter(
    (so) => so.so_no.includes(`SO-${currentYear}-`) && !so.deleted_at
  );

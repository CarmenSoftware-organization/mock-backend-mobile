...
export const createPermission = (permissionData: Omit<Permission, 'id' | 'created_at' | 'updated_at'>): Permission => {
...
export const getPermissionsByActionAndResource = (action: string, resource: string): Permission[] => {
...
export const updatePermission = (id: string, updateData: Partial<Omit<Permission, 'id' | 'created_at' | 'created_by_id'>>): Permission | null => {
...
export const updatePermissionAction = (id: string, action: string): Permission | null => {
  return updatePermission(id, { action });
};

// UPDATE - อัปเดต Permission resource
export const updatePermissionResource = (id: string, resource: string): Permission | null => {
  return updatePermission(id, { resource });
};

// UPDATE - อัปเดต Permission description
export const updatePermissionDescription = (id: string, description: string): Permission | null => {
...
export const deletePermissionsByAction = (action: string): number => {
  ...
  const filteredPermissions = permissions.filter(permission => permission.action !== action);
...
export const deletePermissionsByResource = (resource: string): number => {
  const initialLength = permissions.length;
  const filteredPermissions = permissions.filter(permission => permission.resource !== resource);
  const deletedCount = initialLength - filteredPermissions.length;
...
export const getPermissionCountByAction = (action: string): number => {
  return permissions.filter(permission => permission.action === action).length;
};

// Utility function สำหรับนับจำนวน Permission ตาม resource
export const getPermissionCountByResource = (resource: string): number => {
  return permissions.filter(permission => permission.resource === resource).length;
};
...
export const searchPermissions = (searchCriteria: {
  ...
  return permissions.filter(permission => {
    ...
    if (searchCriteria.resource && permission.resource !== searchCriteria.resource) {
      return false;
    }
    
    if (searchCriteria.description && permission.description && !permission.description.toLowerCase().includes(searchCriteria.description.toLowerCase())) {
      return false;
    }
...
export const isPermissionExists = (action: string, resource: string): boolean => {

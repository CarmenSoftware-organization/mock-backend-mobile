...
export interface Workflow {
  id: string;
  name: string;
  workflow_type: 'purchase_request_workflow' | 'store_requisition_workflow' | 'purchase_order_workflow' | 'grn_workflow' | 'other';
  data: any;
  is_active: boolean;
...
export const workflows: Workflow[] = [
  ...
  },
  {
    ...
    data: { stages: ['draft', 'submitted', 'reviewed', 'approved', 'purchased'] },
    is_active: true,
  ...
  },
  {
    ...
    data: { stages: ['draft', 'submitted', 'approved', 'received', 'completed'] },
    is_active: true,
...
export const createWorkflow = (data: Omit<Workflow, 'id' | 'created_at' | 'created_by_id' | 'updated_at' | 'updated_by_id'>): Workflow => {
...
export const getWorkflowByName = (name: string): Workflow[] => {
  return workflows.filter(workflow => 
    workflow.name.toLowerCase().includes(name.toLowerCase()) && !workflow.deleted_at
  );
};

export const getWorkflowsByType = (workflowType: 'purchase_request_workflow' | 'store_requisition_workflow' | 'purchase_order_workflow' | 'grn_workflow' | 'other'): Workflow[] => {
  return workflows.filter(workflow => workflow.workflow_type === workflowType && !workflow.deleted_at);
};

export const getActiveWorkflows = (): Workflow[] => {
  return workflows.filter(workflow => workflow.is_active && !workflow.deleted_at);
};
...
export const getWorkflowsByCreator = (createdById: string): Workflow[] => {
  return workflows.filter(workflow => workflow.created_by_id === createdById && !workflow.deleted_at);
};

export const getWorkflowsByDateRange = (startDate: string, endDate: string): Workflow[] => {
  return workflows.filter(workflow => {
...
export const getWorkflowsWithDescription = (): Workflow[] => {
  return workflows.filter(workflow => workflow.description !== null && !workflow.deleted_at);
};
...
export const getWorkflowsWithNote = (): Workflow[] => {
  return workflows.filter(workflow => workflow.note !== null && !workflow.deleted_at);
};
...
export const updateWorkflow = (id: string, data: Partial<Omit<Workflow, 'id' | 'created_at' | 'created_by_id'>>): Workflow | null => {
  const index = workflows.findIndex(workflow => workflow.id === id && !workflow.deleted_at);
...
export const updateWorkflowName = (id: string, name: string): Workflow | null => {
  return updateWorkflow(id, { name });
};

// UPDATE - อัปเดต Workflow description
export const updateWorkflowDescription = (id: string, description: string): Workflow | null => {
  return updateWorkflow(id, { description });
};

// UPDATE - อัปเดต Workflow type
export const updateWorkflowType = (id: string, workflowType: 'purchase_request_workflow' | 'store_requisition_workflow' | 'purchase_order_workflow' | 'grn_workflow' | 'other'): Workflow | null => {
  return updateWorkflow(id, { workflow_type: workflowType });
};

// UPDATE - อัปเดต Workflow active status
export const updateWorkflowActiveStatus = (id: string, isActive: boolean): Workflow | null => {
  return updateWorkflow(id, { is_active: isActive });
};

// UPDATE - อัปเดต Workflow note
export const updateWorkflowNote = (id: string, note: string): Workflow | null => {
...
export const updateWorkflowDimension = (id: string, dimension: any): Workflow | null => {
...
export const softDeleteWorkflow = (id: string, deletedById: string): Workflow | null => {
...
export const deleteWorkflowByName = (name: string, deletedById: string): boolean => {
...
export const deleteWorkflowsByType = (workflowType: 'purchase_request_workflow' | 'store_requisition_workflow' | 'purchase_order_workflow' | 'grn_workflow' | 'other', deletedById: string): boolean => {
...
export const searchWorkflows = (criteria: {
  ...
  return workflows.filter(workflow => {
    ...
    if (criteria.name && !workflow.name.toLowerCase().includes(criteria.name.toLowerCase())) {
      return false;
    }
    
    if (criteria.description && workflow.description && !workflow.description.toLowerCase().includes(criteria.description.toLowerCase())) {
      return false;
    }
    
    if (criteria.workflow_type && workflow.workflow_type !== criteria.workflow_type) {
      return false;
    }
    
    if (criteria.is_active !== undefined && workflow.is_active !== criteria.is_active) {
      return false;
    }
    ...
    if (criteria.created_by_id && workflow.created_by_id !== criteria.created_by_id) {
      return false;
    }
...
export const getActiveWorkflowCount = (): number => {
  return workflows.filter(workflow => workflow.is_active && !workflow.deleted_at).length;
};
...
export const getWorkflowCountByType = (workflowType: 'purchase_request_workflow' | 'store_requisition_workflow' | 'purchase_order_workflow' | 'grn_workflow' | 'other'): number => {
  return workflows.filter(workflow => workflow.workflow_type === workflowType && !workflow.deleted_at).length;
};

export const isWorkflowExists = (id: string): boolean => {
  return workflows.some(workflow => workflow.id === id && !workflow.deleted_at);
};

export const isWorkflowNameExists = (name: string): boolean => {
  return workflows.some(workflow => workflow.name === name && !workflow.deleted_at);
};

export const hasActiveWorkflows = (): boolean => {
  return workflows.some(workflow => workflow.is_active && !workflow.deleted_at);
};

export const hasWorkflowsWithDescription = (): boolean => {
  return workflows.some(workflow => workflow.description !== null && !workflow.deleted_at);
};

export const hasWorkflowsWithNote = (): boolean => {
  return workflows.some(workflow => workflow.note !== null && !workflow.deleted_at);
};

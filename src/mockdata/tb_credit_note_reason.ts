...
export const createCreditNoteReason = (creditNoteReasonData: Omit<CreditNoteReason, 'id' | 'created_at' | 'updated_at'>): CreditNoteReason => {
...
export const getCreditNoteReasonById = (id: string): CreditNoteReason | undefined => {
  return creditNoteReasons.find(creditNoteReason => creditNoteReason.id === id);
};
...
export const getCreditNoteReasonByDescription = (description: string): CreditNoteReason[] => {
  return creditNoteReasons.filter(creditNoteReason => 
    creditNoteReason.description.toLowerCase().includes(description.toLowerCase())
  );
};
...
export const updateCreditNoteReason = (id: string, updateData: Partial<Omit<CreditNoteReason, 'id' | 'created_at' | 'created_by_id'>>): CreditNoteReason | null => {
  const index = creditNoteReasons.findIndex(creditNoteReason => creditNoteReason.id === id);
...
export const updateCreditNoteReasonName = (id: string, name: string): CreditNoteReason | null => {
  return updateCreditNoteReason(id, { name });
};

// UPDATE - อัปเดต CreditNoteReason description
export const updateCreditNoteReasonDescription = (id: string, description: string): CreditNoteReason | null => {
  return updateCreditNoteReason(id, { description });
};

// UPDATE - อัปเดต CreditNoteReason note
export const updateCreditNoteReasonNote = (id: string, note: string): CreditNoteReason | null => {
  return updateCreditNoteReason(id, { note });
};

// UPDATE - อัปเดต CreditNoteReason info
export const updateCreditNoteReasonInfo = (id: string, info: any): CreditNoteReason | null => {
  return updateCreditNoteReason(id, { info });
};

// DELETE - ลบ CreditNoteReason (soft delete)
export const deleteCreditNoteReason = (id: string, deletedById: string): boolean => {
  const index = creditNoteReasons.findIndex(creditNoteReason => creditNoteReason.id === id);
...
export const deleteCreditNoteReasonByName = (name: string, deletedById: string): boolean => {
...
export const searchCreditNoteReasons = (searchCriteria: {
  ...
  return creditNoteReasons.filter(creditNoteReason => {
    if (searchCriteria.name && !creditNoteReason.name.toLowerCase().includes(searchCriteria.name.toLowerCase())) {
      return false;
    }
    
    if (searchCriteria.description && !creditNoteReason.description.toLowerCase().includes(searchCriteria.description.toLowerCase())) {
      return false;
    }
    
    if (searchCriteria.note && !creditNoteReason.note.toLowerCase().includes(searchCriteria.note.toLowerCase())) {
      return false;
    }
...
export const isCreditNoteReasonNameExists = (name: string): boolean => {
  return creditNoteReasons.some(creditNoteReason => creditNoteReason.name === name);
};

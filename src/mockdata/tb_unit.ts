...
export const createUnit = (data: Omit<Unit, 'id' | 'created_at' | 'created_by_id' | 'updated_at' | 'updated_by_id'>): Unit => {
  const newUnit: Unit = {
    ...data,
    id: generateId(),
...
export const getUnitsByCreator = (createdById: string): Unit[] => {
  return units.filter(unit => unit.created_by_id === createdById && !unit.deleted_at);
};

export const getUnitsByDateRange = (startDate: string, endDate: string): Unit[] => {
  return units.filter(unit => {
...
// UPDATE - อัปเดต Unit
export const updateUnit = (id: string, data: Partial<Omit<Unit, 'id' | 'created_at' | 'created_by_id'>>): Unit | null => {
...
export const updateUnitStatus = (id: string, isActive: boolean): Unit | null => {
...
export const updateUnitDescription = (id: string, description: string): Unit | null => {
...
export const updateUnitDimension = (id: string, dimension: any): Unit | null => {
  return updateUnit(id, { dimension });
};

// DELETE - Soft delete Unit
export const softDeleteUnit = (id: string, deletedById: string): Unit | null => {
...
export const deleteUnitByName = (name: string, deletedById: string): boolean => {
...
export const searchUnits = (criteria: {
  ...
  return units.filter(unit => {
    ...
    if (criteria.name && !unit.name.toLowerCase().includes(criteria.name.toLowerCase())) {
      return false;
    }
    
    if (criteria.description && !unit.description.toLowerCase().includes(criteria.description.toLowerCase())) {
      return false;
    }
    
    if (criteria.is_active !== undefined && unit.is_active !== criteria.is_active) {
      return false;
    }
    ...
    if (criteria.created_by_id && unit.created_by_id !== criteria.created_by_id) {
      return false;
    }

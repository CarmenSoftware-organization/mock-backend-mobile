...
export const getPurchaseRequestsByCodeAndInProgress = (buCode: string): PurchaseRequest[] => {
  ...
  const result = purchaseRequests.filter(
    (pr) => pr.bu_code === buCode && pr.pr_status === inProgress && !pr.deleted_at
  );
  return result;
};
...
export function swipeApprovePurchaseRequest(id: string) {
  const pr = getPurchaseRequestById(id);
  if (!pr) {
    return null;
  }
  pr.last_action = "approved";
  pr.pr_status = "in_progress";

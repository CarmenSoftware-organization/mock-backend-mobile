...
export const updateMenu = (
  id: string,
  updateData: Partial<Omit<Menu, "id" | "created_at" | "created_by_id">>
): Menu | null => {
...
export const updateMenuActiveStatus = (
  id: string,
  isActive: boolean
): Menu | null => {
...
export const updateMenuLockStatus = (
  id: string,
  isLock: boolean
): Menu | null => {
...
export const updateMenuDescription = (
  id: string,
  description: string
): Menu | null => {
...
export const deleteMenusByModule = (
  moduleId: string,
  deletedById: string
): number => {
  let deletedCount = 0;
...
export const searchMenus = (searchCriteria: {
  ...
  return menus.filter((menu) => {
    if (
      searchCriteria.name &&
      !menu.name.toLowerCase().includes(searchCriteria.name.toLowerCase())
    ) {
      return false;
    }
    ...
    if (
      searchCriteria.url &&
      !menu.url.toLowerCase().includes(searchCriteria.url.toLowerCase())
    ) {
      return false;
    }
    ...
    if (
      ...
      menu.module_id !== searchCriteria.module_id
    ) {
      return false;
    }

    if (
      searchCriteria.is_visible !== undefined &&
      menu.is_visible !== searchCriteria.is_visible
    ) {
      return false;
    }
    ...
    if (
      ...
      menu.is_active !== searchCriteria.is_active
    ) {
      return false;
    }

    if (
      searchCriteria.is_lock !== undefined &&
      menu.is_lock !== searchCriteria.is_lock
    ) {
      return false;
    }

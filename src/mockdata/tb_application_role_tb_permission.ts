...
export const createApplicationRolePermission = (permissionData: Omit<ApplicationRolePermission, 'id' | 'created_at' | 'updated_at'>): ApplicationRolePermission => {
...
export const getAllApplicationRolePermissions = (): ApplicationRolePermission[] => {
  return [...applicationRolePermissions];
};

// READ - อ่าน ApplicationRolePermission ตาม ID
export const getApplicationRolePermissionById = (id: string): ApplicationRolePermission | undefined => {
...
export const getApplicationRolePermissionsByRoleId = (roleId: string): ApplicationRolePermission[] => {
  return applicationRolePermissions.filter(permission => permission.application_role_id === roleId);
};

// READ - อ่าน ApplicationRolePermission ตาม permission_id
export const getApplicationRolePermissionsByPermissionId = (permissionId: string): ApplicationRolePermission[] => {
  return applicationRolePermissions.filter(permission => permission.permission_id === permissionId);
};

// READ - อ่าน ApplicationRolePermission ตาม created_by_id
export const getApplicationRolePermissionsByCreatedBy = (createdById: string): ApplicationRolePermission[] => {
  return applicationRolePermissions.filter(permission => permission.created_by_id === createdById);
};

// READ - อ่าน ApplicationRolePermission ตาม updated_by_id
export const getApplicationRolePermissionsByUpdatedBy = (updatedById: string): ApplicationRolePermission[] => {
  return applicationRolePermissions.filter(permission => permission.updated_by_id === updatedById);
};

// READ - ตรวจสอบว่า role มี permission หรือไม่
export const hasRolePermission = (roleId: string, permissionId: string): boolean => {
  return applicationRolePermissions.some(permission => 
    permission.application_role_id === roleId && permission.permission_id === permissionId
  );
};
...
export const updateApplicationRolePermission = (
  id: string, 
  updateData: Partial<Omit<ApplicationRolePermission, 'id' | 'created_at' | 'created_by_id'>>,
  updatedById: string
): ApplicationRolePermission | null => {
  const index = applicationRolePermissions.findIndex(permission => permission.id === id);
...
export const updateApplicationRolePermissionRoleId = (id: string, roleId: string, updatedById: string): ApplicationRolePermission | null => {
  return updateApplicationRolePermission(id, { application_role_id: roleId }, updatedById);
};

// UPDATE - อัปเดต ApplicationRolePermission permission_id
export const updateApplicationRolePermissionPermissionId = (id: string, permissionId: string, updatedById: string): ApplicationRolePermission | null => {
  return updateApplicationRolePermission(id, { permission_id: permissionId }, updatedById);
};
...
export const updateApplicationRolePermissionByRoleAndPermission = (
  roleId: string, 
  permissionId: string, 
  updateData: Partial<Omit<ApplicationRolePermission, 'id' | 'application_role_id' | 'permission_id' | 'created_at' | 'created_by_id'>>,
  updatedById: string
): ApplicationRolePermission | null => {
  const permission = applicationRolePermissions.find(p => 
    p.application_role_id === roleId && p.permission_id === permissionId
  );
  
  if (!permission) return null;
  
  return updateApplicationRolePermission(permission.id, updateData, updatedById);
};
...
export const deleteApplicationRolePermissionsByRoleId = (roleId: string): number => {
  const initialLength = applicationRolePermissions.length;
  const filteredPermissions = applicationRolePermissions.filter(permission => permission.application_role_id !== roleId);
...
export const deleteApplicationRolePermissionsByPermissionId = (permissionId: string): number => {
  const initialLength = applicationRolePermissions.length;
  const filteredPermissions = applicationRolePermissions.filter(permission => permission.permission_id !== permissionId);
  const deletedCount = initialLength - filteredPermissions.length;
...
export const deleteApplicationRolePermissionByRoleAndPermission = (roleId: string, permissionId: string): boolean => {
  const index = applicationRolePermissions.findIndex(permission => 
    permission.application_role_id === roleId && permission.permission_id === permissionId
  );
...
export const addPermissionsToRole = (roleId: string, permissionIds: string[], createdById: string): ApplicationRolePermission[] => {
  const newPermissions: ApplicationRolePermission[] = [];
  
  permissionIds.forEach(permissionId => {
    // ตรวจสอบว่า permission นี้มีอยู่แล้วหรือไม่
    const exists = applicationRolePermissions.some(p => 
      p.application_role_id === roleId && p.permission_id === permissionId
    );
...
export const removePermissionsFromRole = (roleId: string, permissionIds: string[]): number => {
  let deletedCount = 0;
  
  permissionIds.forEach(permissionId => {
    const deleted = deleteApplicationRolePermissionByRoleAndPermission(roleId, permissionId);
...
export const updateRolePermissions = (roleId: string, permissionIds: string[], updatedById: string): ApplicationRolePermission[] => {
...
export const getPermissionCountByRoleId = (roleId: string): number => {
  return applicationRolePermissions.filter(permission => permission.application_role_id === roleId).length;
};

// Utility function สำหรับนับจำนวน roles ที่มี permission
export const getRoleCountByPermissionId = (permissionId: string): number => {
  return applicationRolePermissions.filter(permission => permission.permission_id === permissionId).length;
};
...
export const searchApplicationRolePermissions = (searchCriteria: {
  ...
  return applicationRolePermissions.filter(permission => {
    ...
    if (searchCriteria.application_role_id && permission.application_role_id !== searchCriteria.application_role_id) {
      return false;
    }
    
    // ตรวจสอบ permission_id
    if (searchCriteria.permission_id && permission.permission_id !== searchCriteria.permission_id) {
      return false;
    }
    
    // ตรวจสอบ created_by_id
    if (searchCriteria.created_by_id && permission.created_by_id !== searchCriteria.created_by_id) {
      return false;
    }
    
    // ตรวจสอบ updated_by_id
    if (searchCriteria.updated_by_id && permission.updated_by_id !== searchCriteria.updated_by_id) {
      return false;
    }
...
export const isApplicationRolePermissionExists = (roleId: string, permissionId: string): boolean => {
  return applicationRolePermissions.some(permission => 
    permission.application_role_id === roleId && permission.permission_id === permissionId
  );
};
...
export const isApplicationRoleExists = (roleId: string): boolean => {
  return applicationRolePermissions.some(permission => permission.application_role_id === roleId);
};

// Utility function สำหรับตรวจสอบ permission_id ซ้ำ
export const isPermissionExists = (permissionId: string): boolean => {
  return applicationRolePermissions.some(permission => permission.permission_id === permissionId);
};

// Utility function สำหรับลบ ApplicationRolePermission ทั้งหมดของ role (hard delete)
export const hardDeleteApplicationRolePermissionsByRoleId = (roleId: string): number => {
  const initialLength = applicationRolePermissions.length;
  const filteredPermissions = applicationRolePermissions.filter(permission => permission.application_role_id !== roleId);
  const deletedCount = initialLength - filteredPermissions.length;
...
export const hardDeleteApplicationRolePermissionsByPermissionId = (permissionId: string): number => {
  const initialLength = applicationRolePermissions.length;
  const filteredPermissions = applicationRolePermissions.filter(permission => permission.permission_id !== permissionId);
  const deletedCount = initialLength - filteredPermissions.length;

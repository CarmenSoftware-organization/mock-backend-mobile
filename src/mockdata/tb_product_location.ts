...
export const createProductLocation = (productLocationData: Omit<ProductLocation, 'id' | 'created_at' | 'updated_at'>): ProductLocation => {
...
export const getProductLocationById = (id: string): ProductLocation | undefined => {
  return productLocations.find(productLocation => productLocation.id === id);
};

// READ - อ่าน ProductLocation ตาม product_id
export const getProductLocationsByProduct = (productId: string): ProductLocation[] => {
  return productLocations.filter(productLocation => productLocation.product_id === productId);
};

// READ - อ่าน ProductLocation ตาม location_id
export const getProductLocationsByLocation = (locationId: string): ProductLocation[] => {
  return productLocations.filter(productLocation => productLocation.location_id === locationId);
};

// READ - อ่าน ProductLocation ตาม product_id และ location_id
export const getProductLocationByProductAndLocation = (productId: string, locationId: string): ProductLocation | undefined => {
  return productLocations.find(productLocation => 
...
export const getProductLocationsBelowReorderQty = (): ProductLocation[] => {
  return productLocations.filter(productLocation => 
    parseFloat(productLocation.par_qty) <= parseFloat(productLocation.re_order_qty)
  );
};
...
export const getProductLocationsByParQtyRange = (minQty: string, maxQty: string): ProductLocation[] => {
  return productLocations.filter(productLocation => {
...
export const searchProductLocations = (searchTerm: string): ProductLocation[] => {
  const lowerSearchTerm = searchTerm.toLowerCase();
  return productLocations.filter(productLocation => 
    (productLocation.note && productLocation.note.toLowerCase().includes(lowerSearchTerm)) ||
...
export const updateProductLocation = (id: string, updateData: Partial<Omit<ProductLocation, 'id' | 'created_at' | 'created_by_id'>>): ProductLocation | null => {
  const index = productLocations.findIndex(productLocation => productLocation.id === id);
...
export const updateProductLocationParQty = (id: string, parQty: string): ProductLocation | null => {
  return updateProductLocation(id, { par_qty: parQty });
};

// UPDATE - อัปเดต ProductLocation note
export const updateProductLocationNote = (id: string, note: string): ProductLocation | null => {
  return updateProductLocation(id, { note });
};

// UPDATE - อัปเดต ProductLocation product
export const updateProductLocationProduct = (id: string, productId: string): ProductLocation | null => {
  return updateProductLocation(id, { product_id: productId });
};

// UPDATE - อัปเดต ProductLocation location
export const updateProductLocationLocation = (id: string, locationId: string): ProductLocation | null => {
  return updateProductLocation(id, { location_id: locationId });
};

// DELETE - ลบ ProductLocation (soft delete)
export const deleteProductLocation = (id: string, deletedById: string): boolean => {
  const index = productLocations.findIndex(productLocation => productLocation.id === id);
...
export const deleteProductLocationsByProduct = (productId: string, deletedById: string): number => {
  let deletedCount = 0;
  
  productLocations.forEach(productLocation => {
    if (productLocation.product_id === productId && !productLocation.deleted_at) {
...
export const deleteProductLocationsByLocation = (locationId: string, deletedById: string): number => {
  let deletedCount = 0;
  
  productLocations.forEach(productLocation => {
    if (productLocation.location_id === locationId && !productLocation.deleted_at) {
      productLocation.deleted_at = getCurrentTimestamp();
      productLocation.deleted_by_id = deletedById;
      deletedCount++;
    }
...
export const deleteProductLocationsBelowMinQty = (deletedById: string): number => {
  let deletedCount = 0;
  
  productLocations.forEach(productLocation => {
    if (parseFloat(productLocation.par_qty) < parseFloat(productLocation.min_qty) && !productLocation.deleted_at) {
      productLocation.deleted_at = getCurrentTimestamp();
      productLocation.deleted_by_id = deletedById;
      deletedCount++;
    }
...
export const getProductLocationCountByProduct = (productId: string): number => {
  return productLocations.filter(productLocation => productLocation.product_id === productId).length;
};

// Utility function สำหรับนับจำนวน ProductLocation ตาม location_id
export const getProductLocationCountByLocation = (locationId: string): number => {
  return productLocations.filter(productLocation => productLocation.location_id === locationId).length;
};

// Utility function สำหรับตรวจสอบ ProductLocation ที่ถูกลบแล้ว
export const getDeletedProductLocations = (): ProductLocation[] => {
  return productLocations.filter(productLocation => productLocation.deleted_at !== null);
};
...
export const searchProductLocationsAdvanced = (searchCriteria: {
  ...
  return productLocations.filter(productLocation => {
    if (searchCriteria.product_id && productLocation.product_id !== searchCriteria.product_id) {
      return false;
    }
    
    if (searchCriteria.location_id && productLocation.location_id !== searchCriteria.location_id) {
      return false;
    }
    
    if (searchCriteria.min_qty && productLocation.min_qty !== searchCriteria.min_qty) {
      return false;
    }
    
    if (searchCriteria.max_qty && productLocation.max_qty !== searchCriteria.max_qty) {
      return false;
    }
    
    if (searchCriteria.re_order_qty && productLocation.re_order_qty !== searchCriteria.re_order_qty) {
      return false;
    }
    
    if (searchCriteria.par_qty && productLocation.par_qty !== searchCriteria.par_qty) {
      return false;
    }
    
    if (searchCriteria.has_note !== undefined) {
      const hasNote = productLocation.note && productLocation.note.trim() !== '';
    ...
    if (searchCriteria.below_reorder_qty !== undefined) {
      const belowReorderQty = parseFloat(productLocation.par_qty) <= parseFloat(productLocation.re_order_qty);
    ...
    if (searchCriteria.below_min_qty !== undefined) {
      const belowMinQty = parseFloat(productLocation.par_qty) < parseFloat(productLocation.min_qty);
    ...
    if (searchCriteria.above_max_qty !== undefined) {
      const aboveMaxQty = parseFloat(productLocation.par_qty) > parseFloat(productLocation.max_qty);

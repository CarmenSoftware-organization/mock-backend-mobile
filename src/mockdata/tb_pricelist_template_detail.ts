...
export const createPricelistTemplateDetail = (
  detailData: Omit<PricelistTemplateDetail, "id" | "created_at" | "updated_at">
...
export const getPricelistTemplateDetailById = (
  id: string
): PricelistTemplateDetail | undefined => {
  return pricelistTemplateDetails.find((detail) => detail.id === id);
};

// READ - อ่าน PricelistTemplateDetail ตาม pricelist_template_id
export const getPricelistTemplateDetailsByTemplateId = (
  templateId: string
): PricelistTemplateDetail[] => {
  return pricelistTemplateDetails.filter(
    (detail) => detail.pricelist_template_id === templateId
  );
};

// READ - อ่าน PricelistTemplateDetail ตาม product_id
export const getPricelistTemplateDetailsByProductId = (
  productId: string
): PricelistTemplateDetail[] => {
  return pricelistTemplateDetails.filter(
...
export const getPricelistTemplateDetailBySequence = (
  templateId: string,
  sequenceNo: number
): PricelistTemplateDetail | undefined => {
  return pricelistTemplateDetails.find(
    (detail) =>
      detail.pricelist_template_id === templateId &&
      detail.sequence_no === sequenceNo
  );
};

// READ - อ่าน PricelistTemplateDetail ตาม product_name
export const getPricelistTemplateDetailsByProductName = (
  productName: string
): PricelistTemplateDetail[] => {
...
export const searchPricelistTemplateDetails = (
  searchTerm: string
): PricelistTemplateDetail[] => {
  ...
  return pricelistTemplateDetails.filter(
    (detail) =>
      ...
      (detail.info?.brand &&
        detail.info.brand.toLowerCase().includes(lowerSearchTerm))
  );
};

// UPDATE - อัปเดต PricelistTemplateDetail
export const updatePricelistTemplateDetail = (
  id: string,
  updateData: Partial<
    Omit<PricelistTemplateDetail, "id" | "created_at" | "created_by_id">
  >
): PricelistTemplateDetail | null => {
  const index = pricelistTemplateDetails.findIndex(
    (detail) => detail.id === id
  );
...
export const updatePricelistTemplateDetailSequence = (
  id: string,
  sequenceNo: number
): PricelistTemplateDetail | null => {
...
export const updatePricelistTemplateDetailProductName = (
  id: string,
  productName: string
): PricelistTemplateDetail | null => {
...
export const updatePricelistTemplateDetailInfo = (
  id: string,
  info: any
): PricelistTemplateDetail | null => {
...
export const deletePricelistTemplateDetail = (
  id: string,
  deletedById: string
): boolean => {
  const index = pricelistTemplateDetails.findIndex(
...
export const deletePricelistTemplateDetailsByTemplateId = (
  templateId: string,
  deletedById: string
): number => {
  let deletedCount = 0;
...
export const deletePricelistTemplateDetailsByProductId = (
  productId: string,
  deletedById: string
): number => {
  let deletedCount = 0;
...
export const getPricelistTemplateDetailCountByTemplateId = (
  templateId: string
): number => {
  return pricelistTemplateDetails.filter(
    (detail) => detail.pricelist_template_id === templateId
...
export const getPricelistTemplateDetailCountByProductId = (
  productId: string
): number => {
  return pricelistTemplateDetails.filter(
    (detail) => detail.product_id === productId
...
export const isSequenceNoExistsInTemplate = (
  templateId: string,
  sequenceNo: number
): boolean => {
  return pricelistTemplateDetails.some(
    (detail) =>
      detail.pricelist_template_id === templateId &&
      detail.sequence_no === sequenceNo
  );
...
export const getDeletedPricelistTemplateDetails =
  (): PricelistTemplateDetail[] => {
    return pricelistTemplateDetails.filter(
...
export const searchPricelistTemplateDetailsAdvanced = (searchCriteria: {
  ...
  return pricelistTemplateDetails.filter((detail) => {
    if (
      searchCriteria.pricelist_template_id &&
      detail.pricelist_template_id !== searchCriteria.pricelist_template_id
    ) {
      return false;
    }
    ...
    if (
      ...
      detail.info?.category !== searchCriteria.category
    ) {
      return false;
    }

...
export const storeRequisitions: StoreRequisition[] = [
  {
    ...
    info: {
      ...
      priority: "high",
      items: ["cleaning supplies", "cooking ingredients"],
    },
...
export const getStoreRequisitionById = (
  id: string
): StoreRequisition | null => {
  const storeRequisition = storeRequisitions.find(
    (sr) => sr.id === id && !sr.deleted_at
  );
...
export const getStoreRequisitionByNo = (
  ...
  const storeRequisition = storeRequisitions.find(
    (sr) => sr.sr_no === srNo && !sr.deleted_at
  );
  return storeRequisition || null;
};

export const getStoreRequisitionsByStatus = (
  status: StoreRequisition["doc_status"]
): StoreRequisition[] => {
  return storeRequisitions.filter(
    (sr) => sr.doc_status === status && !sr.deleted_at
  );
};

export const getStoreRequisitionsByWorkflow = (
  workflowId: string
): StoreRequisition[] => {
  return storeRequisitions.filter(
    (sr) => sr.workflow_id === workflowId && !sr.deleted_at
  );
};

export const getStoreRequisitionsByRequestor = (
  requestorId: string
): StoreRequisition[] => {
  return storeRequisitions.filter(
    (sr) => sr.requestor_id === requestorId && !sr.deleted_at
  );
};

export const getStoreRequisitionsByDepartment = (
  departmentId: string
): StoreRequisition[] => {
  return storeRequisitions.filter(
    (sr) => sr.department_id === departmentId && !sr.deleted_at
  );
};

export const getStoreRequisitionsByLocation = (
  locationId: string
): StoreRequisition[] => {
  return storeRequisitions.filter(
    (sr) => sr.from_location_id === locationId && !sr.deleted_at
  );
};

export const getStoreRequisitionsByDateRange = (
  startDate: string,
  endDate: string
): StoreRequisition[] => {
...
export const getStoreRequisitionsByExpectedDateRange = (
  startDate: string,
  endDate: string
): StoreRequisition[] => {
  return storeRequisitions.filter((sr) => {
...
export const updateStoreRequisition = (
  id: string,
  data: Partial<Omit<StoreRequisition, "id" | "created_at" | "created_by_id">>
): StoreRequisition | null => {
  const index = storeRequisitions.findIndex(
    (sr) => sr.id === id && !sr.deleted_at
  );
...
export const updateStoreRequisitionStatus = (
  id: string,
  status: StoreRequisition["doc_status"]
): StoreRequisition | null => {
...
export const updateStoreRequisitionWorkflow = (
  id: string,
  workflowId: string,
  workflowName: string
): StoreRequisition | null => {
...
export const updateStoreRequisitionDescription = (
  id: string,
  description: string
): StoreRequisition | null => {
...
export const updateStoreRequisitionDates = (
  id: string,
  srDate: string,
  expectedDate: string
): StoreRequisition | null => {
...
export const updateStoreRequisitionLocation = (
  id: string,
  locationId: string,
  locationName: string
): StoreRequisition | null => {
...
export const updateStoreRequisitionRequestor = (
  id: string,
  requestorId: string,
...
export const updateStoreRequisitionDepartment = (
  id: string,
  departmentId: string,
...
export const updateStoreRequisitionInfo = (
  id: string,
  info: any
): StoreRequisition | null => {
...
export const updateStoreRequisitionDimension = (
  id: string,
  dimension: any
): StoreRequisition | null => {
...
export const softDeleteStoreRequisition = (
  id: string,
  deletedById: string
): StoreRequisition | null => {
...
export const restoreStoreRequisition = (
  id: string
): StoreRequisition | null => {
...
export const searchStoreRequisitions = (criteria: {
  ...
  return storeRequisitions.filter((sr) => {
    ...
    if (criteria.workflowId && sr.workflow_id !== criteria.workflowId)
      return false;
    if (criteria.requestorId && sr.requestor_id !== criteria.requestorId)
      return false;
    if (criteria.departmentId && sr.department_id !== criteria.departmentId)
      return false;
    if (criteria.locationId && sr.from_location_id !== criteria.locationId)
      return false;

    if (criteria.startDate || criteria.endDate) {
      const srDate = new Date(sr.sr_date);
      if (criteria.startDate && srDate < new Date(criteria.startDate))
        return false;
    ...
    if (criteria.expectedStartDate || criteria.expectedEndDate) {
      const expectedDate = new Date(sr.expected_date);
      if (
        criteria.expectedStartDate &&
        expectedDate < new Date(criteria.expectedStartDate)
      )
        return false;
      if (
        criteria.expectedEndDate &&
        expectedDate > new Date(criteria.expectedEndDate)
      )
        return false;
    }
    ...
    if (
      ...
      !sr.description.toLowerCase().includes(criteria.description.toLowerCase())
    )
      return false;
...
export const getStoreRequisitionCountByStatus = (
  status: StoreRequisition["doc_status"]
): number => {
  return storeRequisitions.filter(
    (sr) => sr.doc_status === status && !sr.deleted_at
...
export const getNextStoreRequisitionNo = (): string => {
  ...
  const yearStoreRequisitions = storeRequisitions.filter(
    (sr) => sr.sr_no.includes(`SR-${currentYear}-`) && !sr.deleted_at
  );

...
export const createCurrencyComment = (currencyCommentData: Omit<CurrencyComment, 'id' | 'created_at' | 'updated_at'>): CurrencyComment => {
...
export const getCurrencyCommentById = (id: string): CurrencyComment | undefined => {
  return currencyComments.find(currencyComment => currencyComment.id === id);
};

// READ - อ่าน CurrencyComment ตาม type
export const getCurrencyCommentsByType = (type: 'currency' | 'exchange_rate' | 'other'): CurrencyComment[] => {
  return currencyComments.filter(currencyComment => currencyComment.type === type);
};

// READ - อ่าน CurrencyComment ตาม user_id
export const getCurrencyCommentsByUser = (userId: string): CurrencyComment[] => {
  return currencyComments.filter(currencyComment => currencyComment.user_id === userId);
};

// READ - อ่าน CurrencyComment ตาม message
export const getCurrencyCommentsByMessage = (message: string): CurrencyComment[] => {
...
export const getCurrencyCommentsWithAttachments = (): CurrencyComment[] => {
  return currencyComments.filter(currencyComment => currencyComment.attachments !== null);
};
...
export const updateCurrencyComment = (id: string, updateData: Partial<Omit<CurrencyComment, 'id' | 'created_at' | 'created_by_id'>>): CurrencyComment | null => {
  const index = currencyComments.findIndex(currencyComment => currencyComment.id === id);
...
export const updateCurrencyCommentType = (id: string, type: 'currency' | 'exchange_rate' | 'other'): CurrencyComment | null => {
  return updateCurrencyComment(id, { type });
};

// UPDATE - อัปเดต CurrencyComment message
export const updateCurrencyCommentMessage = (id: string, message: string): CurrencyComment | null => {
  return updateCurrencyComment(id, { message });
};

// UPDATE - อัปเดต CurrencyComment note
export const updateCurrencyCommentNote = (id: string, note: string): CurrencyComment | null => {
  return updateCurrencyComment(id, { note });
};

// UPDATE - อัปเดต CurrencyComment attachments
export const updateCurrencyCommentAttachments = (id: string, attachments: any): CurrencyComment | null => {
  return updateCurrencyComment(id, { attachments });
};

// UPDATE - อัปเดต CurrencyComment info
export const updateCurrencyCommentInfo = (id: string, info: any): CurrencyComment | null => {
  return updateCurrencyComment(id, { info });
};

// DELETE - ลบ CurrencyComment (soft delete)
export const deleteCurrencyComment = (id: string, deletedById: string): boolean => {
  const index = currencyComments.findIndex(currencyComment => currencyComment.id === id);
...
export const deleteCurrencyCommentsByType = (type: 'currency' | 'exchange_rate' | 'other', deletedById: string): number => {
  let deletedCount = 0;
...
export const deleteCurrencyCommentsByUser = (userId: string, deletedById: string): number => {
  let deletedCount = 0;
...
export const getCurrencyCommentCountByType = (type: 'currency' | 'exchange_rate' | 'other'): number => {
  return currencyComments.filter(currencyComment => currencyComment.type === type).length;
};
...
export const getCurrencyCommentCountByUser = (userId: string): number => {
  return currencyComments.filter(currencyComment => currencyComment.user_id === userId).length;
};
...
export const searchCurrencyComments = (searchCriteria: {
  ...
  return currencyComments.filter(currencyComment => {
    ...
    if (searchCriteria.user_id && currencyComment.user_id !== searchCriteria.user_id) {
      return false;
    }
    
    if (searchCriteria.message && !currencyComment.message.toLowerCase().includes(searchCriteria.message.toLowerCase())) {
      return false;
    }
    
    if (searchCriteria.note && !currencyComment.note.toLowerCase().includes(searchCriteria.note.toLowerCase())) {
      return false;
    }
...
export const hasCurrencyCommentsByType = (type: 'currency' | 'exchange_rate' | 'other'): boolean => {
  return currencyComments.some(currencyComment => currencyComment.type === type);
};

// Utility function สำหรับตรวจสอบ CurrencyComment ตาม user
export const hasCurrencyCommentsByUser = (userId: string): boolean => {
  return currencyComments.some(currencyComment => currencyComment.user_id === userId);
};

// Utility function สำหรับตรวจสอบ CurrencyComment ที่มี attachments
export const hasCurrencyCommentsWithAttachments = (): boolean => {
  return currencyComments.some(currencyComment => currencyComment.attachments !== null);
};

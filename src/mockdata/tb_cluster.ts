...
export const createCluster = (clusterData: Omit<Cluster, 'id' | 'created_at' | 'updated_at'>): Cluster => {
...
export const updateCluster = (id: string, updateData: Partial<Omit<Cluster, 'id' | 'created_at' | 'created_by_id'>>): Cluster | null => {
...
export const updateClusterStatus = (id: string, isActive: boolean): Cluster | null => {
...
export const updateClusterAliasName = (id: string, aliasName: string): Cluster | null => {
  return updateCluster(id, { alias_name: aliasName });
};

// UPDATE - อัปเดต Cluster logo URL
export const updateClusterLogoUrl = (id: string, logoUrl: string): Cluster | null => {
...
export const searchClusters = (searchCriteria: {
  ...
  return clusters.filter(cluster => {
    if (searchCriteria.code && !cluster.code.toLowerCase().includes(searchCriteria.code.toLowerCase())) {
      return false;
    }
    
    if (searchCriteria.name && !cluster.name.toLowerCase().includes(searchCriteria.name.toLowerCase())) {
      return false;
    }
    
    if (searchCriteria.is_active !== undefined && cluster.is_active !== searchCriteria.is_active) {
      return false;
    }

...
export const createExtraCostDetail = (
  extraCostDetailData: Omit<ExtraCostDetail, "id" | "created_at" | "updated_at">
): ExtraCostDetail => {
...
export const getExtraCostDetailById = (
  id: string
): ExtraCostDetail | undefined => {
  return extraCostDetails.find((extraCostDetail) => extraCostDetail.id === id);
};

// READ - อ่าน ExtraCostDetail ตาม extra_cost_id
export const getExtraCostDetailsByExtraCostId = (
  extraCostId: string
): ExtraCostDetail[] => {
  return extraCostDetails.filter(
    (extraCostDetail) => extraCostDetail.extra_cost_id === extraCostId
  );
};

// READ - อ่าน ExtraCostDetail ตาม extra_cost_type_id
export const getExtraCostDetailsByTypeId = (
  typeId: string
): ExtraCostDetail[] => {
  return extraCostDetails.filter(
    (extraCostDetail) => extraCostDetail.extra_cost_type_id === typeId
  );
};

// READ - อ่าน ExtraCostDetail ตาม sequence_no
export const getExtraCostDetailsBySequenceNo = (
  sequenceNo: number
): ExtraCostDetail[] => {
  return extraCostDetails.filter(
    (extraCostDetail) => extraCostDetail.sequence_no === sequenceNo
  );
};
...
export const getExtraCostDetailsByTaxProfile = (
  taxProfileId: string
): ExtraCostDetail[] => {
  return extraCostDetails.filter(
    (extraCostDetail) => extraCostDetail.tax_profile_id === taxProfileId
  );
};
...
export const getActiveExtraCostDetails = (): ExtraCostDetail[] => {
  return extraCostDetails.filter(
    (extraCostDetail) => extraCostDetail.deleted_at === null
  );
};
...
export const getExtraCostDetailsByDocVersion = (
  docVersion: string
): ExtraCostDetail[] => {
  return extraCostDetails.filter(
    (extraCostDetail) => extraCostDetail.doc_version === docVersion
  );
};

// READ - อ่าน ExtraCostDetail ตาม created_by_id
export const getExtraCostDetailsByCreatedBy = (
  createdById: string
): ExtraCostDetail[] => {
  return extraCostDetails.filter(
    (extraCostDetail) => extraCostDetail.created_by_id === createdById
  );
};

// READ - อ่าน ExtraCostDetail ตาม updated_by_id
export const getExtraCostDetailsByUpdatedBy = (
  updatedById: string
): ExtraCostDetail[] => {
  return extraCostDetails.filter(
    (extraCostDetail) => extraCostDetail.updated_by_id === updatedById
  );
};
...
export const getExtraCostDetailsWithTaxAdjustment = (): ExtraCostDetail[] => {
  return extraCostDetails.filter(
    (extraCostDetail) => extraCostDetail.is_tax_adjustment
  );
};

// READ - อ่าน ExtraCostDetail ที่ไม่มี tax adjustment
export const getExtraCostDetailsWithoutTaxAdjustment =
  (): ExtraCostDetail[] => {
    return extraCostDetails.filter(
      (extraCostDetail) => !extraCostDetail.is_tax_adjustment
    );
...
export const getExtraCostDetailsByAmountRange = (
  minAmount: number,
  maxAmount: number
): ExtraCostDetail[] => {
  return extraCostDetails.filter((extraCostDetail) => {
...
export const updateExtraCostDetail = (
  id: string,
  updateData: Partial<
    Omit<ExtraCostDetail, "id" | "created_at" | "created_by_id">
  >
): ExtraCostDetail | null => {
  const index = extraCostDetails.findIndex(
    (extraCostDetail) => extraCostDetail.id === id
  );
...
export const updateExtraCostDetailSequenceNo = (
  id: string,
  sequenceNo: number
): ExtraCostDetail | null => {
...
export const updateExtraCostDetailName = (
  id: string,
  name: string
): ExtraCostDetail | null => {
...
export const updateExtraCostDetailDescription = (
  id: string,
  description: string
): ExtraCostDetail | null => {
...
export const updateExtraCostDetailNote = (
  id: string,
  note: string
): ExtraCostDetail | null => {
...
export const updateExtraCostDetailAmount = (
  id: string,
  amount: number
): ExtraCostDetail | null => {
...
export const updateExtraCostDetailTaxInfo = (
  id: string,
  taxProfileId: string,
...
export const updateExtraCostDetailTaxAdjustment = (
  id: string,
...
export const updateExtraCostDetailInfo = (
  id: string,
  info: any
): ExtraCostDetail | null => {
...
export const updateExtraCostDetailDimension = (
  id: string,
  dimension: any
): ExtraCostDetail | null => {
...
export const updateExtraCostDetailDocVersion = (
  id: string,
  docVersion: string
): ExtraCostDetail | null => {
...
export const updateExtraCostDetailByExtraCostAndSequence = (
  extraCostId: string,
  sequenceNo: number,
  updateData: Partial<
    Omit<ExtraCostDetail, "id" | "created_at" | "created_by_id">
  ...
  const index = extraCostDetails.findIndex(
    (ecd) => ecd.extra_cost_id === extraCostId && ecd.sequence_no === sequenceNo
  );
...
export const deleteExtraCostDetail = (
  id: string,
  deletedById: string
): boolean => {
  const index = extraCostDetails.findIndex(
...
export const deleteExtraCostDetailsByExtraCostId = (
  extraCostId: string,
  deletedById: string
): number => {
  let deletedCount = 0;
  ...
  extraCostDetails.forEach((extraCostDetail) => {
    if (
      extraCostDetail.extra_cost_id === extraCostId &&
      !extraCostDetail.deleted_at
    ) {
...
export const deleteExtraCostDetailsByTypeId = (
  typeId: string,
  deletedById: string
): number => {
  let deletedCount = 0;
  ...
  extraCostDetails.forEach((extraCostDetail) => {
    if (
      extraCostDetail.extra_cost_type_id === typeId &&
      !extraCostDetail.deleted_at
    ) {
      extraCostDetail.deleted_at = getCurrentTimestamp();
      extraCostDetail.deleted_by_id = deletedById;
...
export const deleteExtraCostDetailByExtraCostAndSequence = (
  extraCostId: string,
  sequenceNo: number,
  deletedById: string
): boolean => {
...
export const restoreExtraCostDetailsByExtraCostId = (
  extraCostId: string
): number => {
  let restoredCount = 0;
...
export const getExtraCostDetailCountByExtraCostId = (
  extraCostId: string
): number => {
  return extraCostDetails.filter(
    (extraCostDetail) => extraCostDetail.extra_cost_id === extraCostId
...
export const getExtraCostDetailCountByTaxProfile = (
  taxProfileId: string
): number => {
  return extraCostDetails.filter(
    (extraCostDetail) => extraCostDetail.tax_profile_id === taxProfileId
...
export const getExtraCostDetailCountByDocVersion = (
  docVersion: string
): number => {
  return extraCostDetails.filter(
    (extraCostDetail) => extraCostDetail.doc_version === docVersion
...
export const getExtraCostDetailCountByCreatedBy = (
  createdById: string
): number => {
  return extraCostDetails.filter(
    (extraCostDetail) => extraCostDetail.created_by_id === createdById
...
export const searchExtraCostDetails = (searchCriteria: {
  ...
  return extraCostDetails.filter((extraCostDetail) => {
    if (
      searchCriteria.extra_cost_id &&
      extraCostDetail.extra_cost_id !== searchCriteria.extra_cost_id
    ) {
      return false;
    }

    if (
      searchCriteria.extra_cost_type_id &&
      extraCostDetail.extra_cost_type_id !== searchCriteria.extra_cost_type_id
    ) {
      return false;
    }
    ...
    if (
      ...
      extraCostDetail.sequence_no !== searchCriteria.sequence_no
    ) {
      return false;
    }
    ...
    if (
      ...
      extraCostDetail.tax_profile_id !== searchCriteria.tax_profile_id
    ) {
      return false;
    }

    if (
      searchCriteria.is_tax_adjustment !== undefined &&
      extraCostDetail.is_tax_adjustment !== searchCriteria.is_tax_adjustment
    ) {
      return false;
    }
    ...
    if (
      ...
      extraCostDetail.doc_version !== searchCriteria.doc_version
    ) {
      return false;
    }
    ...
    if (
      ...
      extraCostDetail.created_by_id !== searchCriteria.created_by_id
    ) {
      return false;
    }
    ...
    if (
      ...
      extraCostDetail.updated_by_id !== searchCriteria.updated_by_id
    ) {
      return false;
    }
    ...
    if (searchCriteria.has_note !== undefined) {
      const hasNote =
        extraCostDetail.note && extraCostDetail.note.trim() !== "";
...
export const isExtraCostDetailExistsByExtraCostId = (
  extraCostId: string
): boolean => {
  return extraCostDetails.some(
    (extraCostDetail) => extraCostDetail.extra_cost_id === extraCostId
  );
};
...
export const isExtraCostDetailExistsByExtraCostAndSequence = (
  extraCostId: string,
  sequenceNo: number
): boolean => {
  return extraCostDetails.some(
...
export const hasExtraCostDetailsWithInfo = (): boolean => {
  return extraCostDetails.some(
    (extraCostDetail) => extraCostDetail.info !== null
  );
};
...
export const hasExtraCostDetailsWithDimension = (): boolean => {
  return extraCostDetails.some(
    (extraCostDetail) => extraCostDetail.dimension !== null
  );
};

// Bulk operations
// เพิ่ม ExtraCostDetail หลายรายการ
export const addMultipleExtraCostDetails = (
  extraCostDetailsData: Omit<
    ExtraCostDetail,
...
export const removeMultipleExtraCostDetailsByExtraCostId = (
  extraCostId: string,
  deletedById: string
): number => {
...
export const removeMultipleExtraCostDetailsByTypeId = (
  typeId: string,
  deletedById: string
): number => {
...
export const hardDeleteExtraCostDetailsByExtraCostId = (
  extraCostId: string
): number => {
  let deletedCount = 0;
...
export const getNextExtraCostSequenceNo = (extraCostId: string): number => {
  const existingDetails = extraCostDetails.filter(
    (ecd) => ecd.extra_cost_id === extraCostId
  );
  ...
  const maxSequenceNo = Math.max(
    ...existingDetails.map((ecd) => ecd.sequence_no)
  );
  return maxSequenceNo + 1;
};

export const isSequenceNoExistsInExtraCost = (
  extraCostId: string,
  sequenceNo: number
): boolean => {
  return extraCostDetails.some(

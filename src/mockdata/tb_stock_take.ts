...
export const stockTakes: StockTake[] = [
  {
    ...
    info: { category: 'IT Equipment', priority: 'high', location: 'IT Storage Room' },
  ...
  },
  {
    ...
    info: { category: 'Office Supplies', priority: 'medium', location: 'Admin Storage' },
  ...
  },
  {
    ...
    stk_no: 'STK-2024-003',
    description: 'Quarterly stock take for marketing materials and promotional items',
    doc_status: 'draft',
    workflow_id: 'wf-003',
    workflow_name: 'Stock Take Approval',
    workflow_history: [
    ...
    info: { category: 'Marketing', priority: 'low', location: 'Marketing Storage' },
...
export const createStockTake = (data: Omit<StockTake, 'id' | 'created_at' | 'created_by_id' | 'updated_at' | 'updated_by_id'>): StockTake => {
...
export const getStockTakeByNo = (stkNo: string): StockTake | null => {
  const stockTake = stockTakes.find(st => st.stk_no === stkNo && !st.deleted_at);
  return stockTake || null;
};

export const getStockTakesByStatus = (status: StockTake['doc_status']): StockTake[] => {
...
export const getStockTakesByWorkflow = (workflowId: string): StockTake[] => {
  return stockTakes.filter(st => st.workflow_id === workflowId && !st.deleted_at);
};

export const getStockTakesByCreator = (createdById: string): StockTake[] => {
  return stockTakes.filter(st => st.created_by_id === createdById && !st.deleted_at);
};

export const getStockTakesByDateRange = (startDate: string, endDate: string): StockTake[] => {
...
export const getStockTakesByName = (name: string): StockTake[] => {
  return stockTakes.filter(st => 
    st.name.toLowerCase().includes(name.toLowerCase()) && !st.deleted_at
  );
};

// UPDATE - อัปเดต StockTake
export const updateStockTake = (id: string, data: Partial<Omit<StockTake, 'id' | 'created_at' | 'created_by_id'>>): StockTake | null => {
...
export const updateStockTakeName = (id: string, name: string): StockTake | null => {
  return updateStockTake(id, { name });
};

// UPDATE - อัปเดต StockTake description
export const updateStockTakeDescription = (id: string, description: string): StockTake | null => {
  return updateStockTake(id, { description });
};

// UPDATE - อัปเดต StockTake status
export const updateStockTakeStatus = (id: string, status: StockTake['doc_status']): StockTake | null => {
  return updateStockTake(id, { doc_status: status });
};

// UPDATE - อัปเดต StockTake workflow
export const updateStockTakeWorkflow = (id: string, workflowId: string, workflowName: string): StockTake | null => {
...
export const updateStockTakeNote = (id: string, note: string): StockTake | null => {
  return updateStockTake(id, { note });
};

// UPDATE - อัปเดต StockTake info
export const updateStockTakeInfo = (id: string, info: any): StockTake | null => {
  return updateStockTake(id, { info });
};

// UPDATE - อัปเดต StockTake dimension
export const updateStockTakeDimension = (id: string, dimension: any): StockTake | null => {
  return updateStockTake(id, { dimension });
};

// DELETE - Soft delete StockTake
export const softDeleteStockTake = (id: string, deletedById: string): StockTake | null => {
...
export const searchStockTakes = (criteria: {
  ...
  return stockTakes.filter(st => {
    ...
    if (criteria.workflowId && st.workflow_id !== criteria.workflowId) return false;
    if (criteria.createdById && st.created_by_id !== criteria.createdById) return false;
    ...
    if (criteria.name && !st.name.toLowerCase().includes(criteria.name.toLowerCase())) return false;
    if (criteria.description && !st.description.toLowerCase().includes(criteria.description.toLowerCase())) return false;
    if (criteria.note && st.note && !st.note.toLowerCase().includes(criteria.note.toLowerCase())) return false;
...
export const getStockTakeCountByStatus = (status: StockTake['doc_status']): number => {
  return stockTakes.filter(st => st.doc_status === status && !st.deleted_at).length;
};
...
export const getNextStockTakeNo = (): string => {
  ...
  const yearStockTakes = stockTakes.filter(st => 
    st.stk_no.includes(`STK-${currentYear}-`) && !st.deleted_at
  );

...
export const stockOutDetails: StockOutDetail[] = [
  {
    ...
    info: { recipient: 'Developer Team', purpose: 'Development Work', warranty: '3 years' },
  ...
  },
  {
    ...
    qty: '3.00',
    note: 'Distributed to new developers',
    info: { recipient: 'Developer Team', purpose: 'Development Work', resolution: '4K UHD' },
    dimension: { cost_center: 'IT-001', project: 'New Hire Setup' },
    doc_version: '1',
    created_at: '2024-01-10T09:00:00Z',
    created_by_id: 'user-001',
  ...
  },
  {
    ...
    info: { recipient: 'Multiple Departments', purpose: 'Office Setup', material: 'Mesh' },
  ...
  },
  {
    ...
    qty: '20.00',
    note: 'Distributed to various departments',
    info: { recipient: 'Multiple Departments', purpose: 'Office Setup', power: '10W LED' },
    dimension: { cost_center: 'ADMIN-001', project: 'Department Supplies' },
    doc_version: '1',
    created_at: '2024-01-18T08:00:00Z',
    created_by_id: 'user-004',
...
export const createStockOutDetail = (data: Omit<StockOutDetail, 'id' | 'created_at' | 'created_by_id' | 'updated_at' | 'updated_by_id'>): StockOutDetail => {
...
export const getStockOutDetailsByStockOutId = (stockOutId: string): StockOutDetail[] => {
  return stockOutDetails.filter(d => d.stock_out_id === stockOutId && !d.deleted_at);
};

export const getStockOutDetailsByInventoryTransactionId = (inventoryTransactionId: string): StockOutDetail[] => {
  return stockOutDetails.filter(d => d.inventory_transaction_id === inventoryTransactionId && !d.deleted_at);
};

export const getStockOutDetailsByProductId = (productId: string): StockOutDetail[] => {
  return stockOutDetails.filter(d => d.product_id === productId && !d.deleted_at);
};

export const getStockOutDetailsByLocationId = (locationId: string): StockOutDetail[] => {
  return stockOutDetails.filter(d => d.location_id === locationId && !d.deleted_at);
};

export const getStockOutDetailsBySequenceNo = (stockOutId: string, sequenceNo: number): StockOutDetail | null => {
  const detail = stockOutDetails.find(d => 
...
export const updateStockOutDetail = (id: string, data: Partial<Omit<StockOutDetail, 'id' | 'created_at' | 'created_by_id'>>): StockOutDetail | null => {
...
export const updateStockOutDetailDescription = (id: string, description: string): StockOutDetail | null => {
  return updateStockOutDetail(id, { description });
};

// UPDATE - อัปเดต StockOutDetail qty
export const updateStockOutDetailQty = (id: string, qty: string): StockOutDetail | null => {
  return updateStockOutDetail(id, { qty });
};

// UPDATE - อัปเดต StockOutDetail note
export const updateStockOutDetailNote = (id: string, note: string): StockOutDetail | null => {
  return updateStockOutDetail(id, { note });
};

// UPDATE - อัปเดต StockOutDetail info
export const updateStockOutDetailInfo = (id: string, info: any): StockOutDetail | null => {
  return updateStockOutDetail(id, { info });
};

// UPDATE - อัปเดต StockOutDetail dimension
export const updateStockOutDetailDimension = (id: string, dimension: any): StockOutDetail | null => {
  return updateStockOutDetail(id, { dimension });
};

// UPDATE - อัปเดต StockOutDetail location
export const updateStockOutDetailLocation = (id: string, locationId: string, locationName: string): StockOutDetail | null => {
...
export const updateStockOutDetailProduct = (id: string, productId: string, productName: string, productLocalName: string): StockOutDetail | null => {
  return updateStockOutDetail(id, { 
...
export const softDeleteStockOutDetail = (id: string, deletedById: string): StockOutDetail | null => {
...
export const deleteStockOutDetailsByStockOutId = (stockOutId: string, deletedById: string): number => {
  let deletedCount = 0;
...
export const searchStockOutDetails = (criteria: {
  ...
  return stockOutDetails.filter(detail => {
    ...
    if (criteria.stockOutId && detail.stock_out_id !== criteria.stockOutId) return false;
...
export const getStockOutDetailCountByStockOutId = (stockOutId: string): number => {
  return stockOutDetails.filter(detail => detail.stock_out_id === stockOutId && !detail.deleted_at).length;
};

export const isStockOutDetailExists = (id: string): boolean => {
  return stockOutDetails.some(detail => detail.id === id && !detail.deleted_at);
};

export const isSequenceNoExists = (stockOutId: string, sequenceNo: number): boolean => {
  return stockOutDetails.some(detail => 

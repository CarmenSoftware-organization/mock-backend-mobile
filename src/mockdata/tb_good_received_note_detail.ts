...
export interface GoodReceivedNoteDetail {
  ...
  export const createGoodReceivedNoteDetail = (goodReceivedNoteDetailData: Omit<GoodReceivedNoteDetail, 'id' | 'created_at' | 'updated_at'>): GoodReceivedNoteDetail => {
  ...
  export const getGoodReceivedNoteDetailById = (id: string): GoodReceivedNoteDetail | undefined => {
    return goodReceivedNoteDetails.find(detail => detail.id === id);
  };

  // READ - อ่าน GoodReceivedNoteDetail ตาม good_received_note_id
  export const getGoodReceivedNoteDetailsByGrnId = (grnId: string): GoodReceivedNoteDetail[] => {
    return goodReceivedNoteDetails.filter(detail => detail.good_received_note_id === grnId);
  };

  // READ - อ่าน GoodReceivedNoteDetail ตาม inventory_transaction_id
  export const getGoodReceivedNoteDetailsByInventoryTransactionId = (inventoryTransactionId: string): GoodReceivedNoteDetail[] => {
    return goodReceivedNoteDetails.filter(detail => detail.inventory_transaction_id === inventoryTransactionId);
  };

  // READ - อ่าน GoodReceivedNoteDetail ตาม purchase_order_detail_id
  export const getGoodReceivedNoteDetailsByPurchaseOrderDetailId = (purchaseOrderDetailId: string): GoodReceivedNoteDetail[] => {
    return goodReceivedNoteDetails.filter(detail => detail.purchase_order_detail_id === purchaseOrderDetailId);
  };

  // READ - อ่าน GoodReceivedNoteDetail ตาม sequence_no
  export const getGoodReceivedNoteDetailsBySequenceNo = (sequenceNo: number): GoodReceivedNoteDetail[] => {
    return goodReceivedNoteDetails.filter(detail => detail.sequence_no === sequenceNo);
  };

  // READ - อ่าน GoodReceivedNoteDetail ตาม location_id
  export const getGoodReceivedNoteDetailsByLocationId = (locationId: string): GoodReceivedNoteDetail[] => {
    return goodReceivedNoteDetails.filter(detail => detail.location_id === locationId);
  };

  // READ - อ่าน GoodReceivedNoteDetail ตาม product_id
  export const getGoodReceivedNoteDetailsByProductId = (productId: string): GoodReceivedNoteDetail[] => {
    return goodReceivedNoteDetails.filter(detail => detail.product_id === productId);
  };

  // READ - อ่าน GoodReceivedNoteDetail ตาม product_name
  export const getGoodReceivedNoteDetailsByProductName = (productName: string): GoodReceivedNoteDetail[] => {
  ...
  export const getGoodReceivedNoteDetailsByTaxProfile = (taxProfileId: string): GoodReceivedNoteDetail[] => {
    return goodReceivedNoteDetails.filter(detail => detail.tax_profile_id === taxProfileId);
  };

  // READ - อ่าน GoodReceivedNoteDetail ตาม delivery_point_id
  export const getGoodReceivedNoteDetailsByDeliveryPoint = (deliveryPointId: string): GoodReceivedNoteDetail[] => {
    return goodReceivedNoteDetails.filter(detail => detail.delivery_point_id === deliveryPointId);
  };

  // READ - อ่าน GoodReceivedNoteDetail ที่ active (ไม่ถูกลบ)
  export const getActiveGoodReceivedNoteDetails = (): GoodReceivedNoteDetail[] => {
    return goodReceivedNoteDetails.filter(detail => detail.deleted_at === null);
  };

  // READ - อ่าน GoodReceivedNoteDetail ที่ถูกลบแล้ว
  export const getDeletedGoodReceivedNoteDetails = (): GoodReceivedNoteDetail[] => {
    return goodReceivedNoteDetails.filter(detail => detail.deleted_at !== null);
  };

  // READ - อ่าน GoodReceivedNoteDetail ตาม doc_version
  export const getGoodReceivedNoteDetailsByDocVersion = (docVersion: string): GoodReceivedNoteDetail[] => {
    return goodReceivedNoteDetails.filter(detail => detail.doc_version === docVersion);
  };

  // READ - อ่าน GoodReceivedNoteDetail ตาม created_by_id
  export const getGoodReceivedNoteDetailsByCreatedBy = (createdById: string): GoodReceivedNoteDetail[] => {
    return goodReceivedNoteDetails.filter(detail => detail.created_by_id === createdById);
  };

  // READ - อ่าน GoodReceivedNoteDetail ตาม updated_by_id
  export const getGoodReceivedNoteDetailsByUpdatedBy = (updatedById: string): GoodReceivedNoteDetail[] => {
    return goodReceivedNoteDetails.filter(detail => detail.updated_by_id === updatedById);
  };

  // READ - อ่าน GoodReceivedNoteDetail ที่มี note
  export const getGoodReceivedNoteDetailsWithNote = (): GoodReceivedNoteDetail[] => {
    return goodReceivedNoteDetails.filter(detail => detail.note && detail.note.trim() !== '');
  };

  // READ - อ่าน GoodReceivedNoteDetail ที่มี FOC qty
  export const getGoodReceivedNoteDetailsWithFoc = (): GoodReceivedNoteDetail[] => {
    return goodReceivedNoteDetails.filter(detail => detail.foc_qty > 0);
  };

  // READ - อ่าน GoodReceivedNoteDetail ตาม price range
  export const getGoodReceivedNoteDetailsByPriceRange = (minPrice: number, maxPrice: number): GoodReceivedNoteDetail[] => {
    return goodReceivedNoteDetails.filter(detail => {
  ...
  export const getGoodReceivedNoteDetailsByQtyRange = (minQty: number, maxQty: number): GoodReceivedNoteDetail[] => {
    return goodReceivedNoteDetails.filter(detail => {
  ...
  export const updateGoodReceivedNoteDetail = (id: string, updateData: Partial<Omit<GoodReceivedNoteDetail, 'id' | 'created_at' | 'created_by_id'>>): GoodReceivedNoteDetail | null => {
  ...
  export const updateGoodReceivedNoteDetailSequenceNo = (id: string, sequenceNo: number): GoodReceivedNoteDetail | null => {
    return updateGoodReceivedNoteDetail(id, { sequence_no: sequenceNo });
  };

  // UPDATE - อัปเดต GoodReceivedNoteDetail received_qty
  export const updateGoodReceivedNoteDetailReceivedQty = (id: string, receivedQty: number): GoodReceivedNoteDetail | null => {
    return updateGoodReceivedNoteDetail(id, { received_qty: receivedQty });
  };

  // UPDATE - อัปเดต GoodReceivedNoteDetail price
  export const updateGoodReceivedNoteDetailPrice = (id: string, price: number): GoodReceivedNoteDetail | null => {
    return updateGoodReceivedNoteDetail(id, { price });
  };

  // UPDATE - อัปเดต GoodReceivedNoteDetail note
  export const updateGoodReceivedNoteDetailNote = (id: string, note: string): GoodReceivedNoteDetail | null => {
    return updateGoodReceivedNoteDetail(id, { note });
  };

  // UPDATE - อัปเดต GoodReceivedNoteDetail info
  export const updateGoodReceivedNoteDetailInfo = (id: string, info: any): GoodReceivedNoteDetail | null => {
    return updateGoodReceivedNoteDetail(id, { info });
  };

  // UPDATE - อัปเดต GoodReceivedNoteDetail dimension
  export const updateGoodReceivedNoteDetailDimension = (id: string, dimension: any): GoodReceivedNoteDetail | null => {
    return updateGoodReceivedNoteDetail(id, { dimension });
  };

  // UPDATE - อัปเดต GoodReceivedNoteDetail doc_version
  export const updateGoodReceivedNoteDetailDocVersion = (id: string, docVersion: string): GoodReceivedNoteDetail | null => {
  ...
  export const updateGoodReceivedNoteDetailByGrnAndSequence = (grnId: string, sequenceNo: number, updateData: Partial<Omit<GoodReceivedNoteDetail, 'id' | 'created_at' | 'created_by_id'>>): GoodReceivedNoteDetail | null => {
    const index = goodReceivedNoteDetails.findIndex(detail => detail.good_received_note_id === grnId && detail.sequence_no === sequenceNo);
  ...
  export const deleteGoodReceivedNoteDetail = (id: string, deletedById: string): boolean => {
    const index = goodReceivedNoteDetails.findIndex(detail => detail.id === id);
  ...
  export const deleteGoodReceivedNoteDetailsByGrnId = (grnId: string, deletedById: string): number => {
    let deletedCount = 0;
  ...
  export const deleteGoodReceivedNoteDetailsByInventoryTransactionId = (inventoryTransactionId: string, deletedById: string): number => {
    let deletedCount = 0;
    
    goodReceivedNoteDetails.forEach(detail => {
      if (detail.inventory_transaction_id === inventoryTransactionId && !detail.deleted_at) {
        detail.deleted_at = getCurrentTimestamp();
        detail.deleted_by_id = deletedById;
        deletedCount++;
      }
  ...
  export const deleteGoodReceivedNoteDetailsByProductId = (productId: string, deletedById: string): number => {
    let deletedCount = 0;
  ...
  export const deleteGoodReceivedNoteDetailByGrnAndSequence = (grnId: string, sequenceNo: number, deletedById: string): boolean => {
    const index = goodReceivedNoteDetails.findIndex(detail => detail.good_received_note_id === grnId && detail.sequence_no === sequenceNo);
  ...
  export const restoreGoodReceivedNoteDetailsByGrnId = (grnId: string): number => {
    let restoredCount = 0;
  ...
  export const restoreGoodReceivedNoteDetailsByInventoryTransactionId = (inventoryTransactionId: string): number => {
    let restoredCount = 0;
  ...
  export const restoreGoodReceivedNoteDetailsByProductId = (productId: string): number => {
    let restoredCount = 0;
  ...
  export const getActiveGoodReceivedNoteDetailCount = (): number => {
    return goodReceivedNoteDetails.filter(detail => detail.deleted_at === null).length;
  };
  ...
  export const getGoodReceivedNoteDetailCountByGrnId = (grnId: string): number => {
    return goodReceivedNoteDetails.filter(detail => detail.good_received_note_id === grnId).length;
  };

  // Utility function สำหรับนับจำนวน GoodReceivedNoteDetail ตาม inventory_transaction_id
  export const getGoodReceivedNoteDetailCountByInventoryTransactionId = (inventoryTransactionId: string): number => {
    return goodReceivedNoteDetails.filter(detail => detail.inventory_transaction_id === inventoryTransactionId).length;
  };

  // Utility function สำหรับนับจำนวน GoodReceivedNoteDetail ตาม product_id
  export const getGoodReceivedNoteDetailCountByProductId = (productId: string): number => {
    return goodReceivedNoteDetails.filter(detail => detail.product_id === productId).length;
  };

  // Utility function สำหรับนับจำนวน GoodReceivedNoteDetail ตาม location_id
  export const getGoodReceivedNoteDetailCountByLocationId = (locationId: string): number => {
    return goodReceivedNoteDetails.filter(detail => detail.location_id === locationId).length;
  };

  // Utility function สำหรับนับจำนวน GoodReceivedNoteDetail ตาม tax_profile_id
  export const getGoodReceivedNoteDetailCountByTaxProfile = (taxProfileId: string): number => {
    return goodReceivedNoteDetails.filter(detail => detail.tax_profile_id === taxProfileId).length;
  };

  // Utility function สำหรับนับจำนวน GoodReceivedNoteDetail ตาม delivery_point_id
  export const getGoodReceivedNoteDetailCountByDeliveryPoint = (deliveryPointId: string): number => {
    return goodReceivedNoteDetails.filter(detail => detail.delivery_point_id === deliveryPointId).length;
  };

  // Utility function สำหรับนับจำนวน GoodReceivedNoteDetail ตาม doc_version
  export const getGoodReceivedNoteDetailCountByDocVersion = (docVersion: string): number => {
    return goodReceivedNoteDetails.filter(detail => detail.doc_version === docVersion).length;
  };

  // Utility function สำหรับนับจำนวน GoodReceivedNoteDetail ตาม created_by_id
  export const getGoodReceivedNoteDetailCountByCreatedBy = (createdById: string): number => {
    return goodReceivedNoteDetails.filter(detail => detail.created_by_id === createdById).length;
  };
  ...
  export const searchGoodReceivedNoteDetails = (searchCriteria: {
    ...
    return goodReceivedNoteDetails.filter(detail => {
      if (searchCriteria.good_received_note_id && detail.good_received_note_id !== searchCriteria.good_received_note_id) {
        return false;
      }
      ...
      if (searchCriteria.purchase_order_detail_id && detail.purchase_order_detail_id !== searchCriteria.purchase_order_detail_id) {
        return false;
      }
      
      if (searchCriteria.sequence_no !== undefined && detail.sequence_no !== searchCriteria.sequence_no) {
        return false;
      }
      ...
      if (searchCriteria.delivery_point_id && detail.delivery_point_id !== searchCriteria.delivery_point_id) {
        return false;
      }
      
      if (searchCriteria.doc_version && detail.doc_version !== searchCriteria.doc_version) {
        return false;
      }
  ...
  export const isGoodReceivedNoteDetailExistsByGrnId = (grnId: string): boolean => {
    return goodReceivedNoteDetails.some(detail => detail.good_received_note_id === grnId);
  };

  // Utility function สำหรับตรวจสอบ GoodReceivedNoteDetail ตาม inventory_transaction_id
  export const isGoodReceivedNoteDetailExistsByInventoryTransactionId = (inventoryTransactionId: string): boolean => {
    return goodReceivedNoteDetails.some(detail => detail.inventory_transaction_id === inventoryTransactionId);
  };

  // Utility function สำหรับตรวจสอบ GoodReceivedNoteDetail ตาม product_id
  export const isGoodReceivedNoteDetailExistsByProductId = (productId: string): boolean => {
    return goodReceivedNoteDetails.some(detail => detail.product_id === productId);
  };

  // Utility function สำหรับตรวจสอบ GoodReceivedNoteDetail โดย good_received_note_id และ sequence_no
  export const isGoodReceivedNoteDetailExistsByGrnAndSequence = (grnId: string, sequenceNo: number): boolean => {
    return goodReceivedNoteDetails.some(detail => detail.good_received_note_id === grnId && detail.sequence_no === sequenceNo);
  };

  // Utility function สำหรับตรวจสอบ GoodReceivedNoteDetail ที่มี note
  export const hasGoodReceivedNoteDetailsWithNote = (): boolean => {
    return goodReceivedNoteDetails.some(detail => detail.note && detail.note.trim() !== '');
  };
  ...
  export const addMultipleGoodReceivedNoteDetails = (goodReceivedNoteDetailsData: Omit<GoodReceivedNoteDetail, 'id' | 'created_at' | 'updated_at'>[]): GoodReceivedNoteDetail[] => {
  ...
  export const removeMultipleGoodReceivedNoteDetailsByGrnId = (grnId: string, deletedById: string): number => {
  ...
  export const removeMultipleGoodReceivedNoteDetailsByInventoryTransactionId = (inventoryTransactionId: string, deletedById: string): number => {
    return deleteGoodReceivedNoteDetailsByInventoryTransactionId(inventoryTransactionId, deletedById);
  };

  // ลบ GoodReceivedNoteDetail หลายรายการตาม product_id
  export const removeMultipleGoodReceivedNoteDetailsByProductId = (productId: string, deletedById: string): number => {
    return deleteGoodReceivedNoteDetailsByProductId(productId, deletedById);
  };

  // Hard delete operations
  export const hardDeleteGoodReceivedNoteDetailsByGrnId = (grnId: string): number => {
    let deletedCount = 0;
  ...
  export const hardDeleteGoodReceivedNoteDetailsByInventoryTransactionId = (inventoryTransactionId: string): number => {
    let deletedCount = 0;
    
    for (let i = goodReceivedNoteDetails.length - 1; i >= 0; i--) {
      if (goodReceivedNoteDetails[i].inventory_transaction_id === inventoryTransactionId) {
        goodReceivedNoteDetails.splice(i, 1);
        deletedCount++;
      }
    }
  ...
  export const hardDeleteGoodReceivedNoteDetailsByProductId = (productId: string): number => {
    let deletedCount = 0;
  ...
  export const getNextGoodReceivedNoteSequenceNo = (grnId: string): number => {
    const existingDetails = goodReceivedNoteDetails.filter(detail => detail.good_received_note_id === grnId);
    if (existingDetails.length === 0) {
      return 1;
    }
    
    const maxSequenceNo = Math.max(...existingDetails.map(detail => detail.sequence_no));
    return maxSequenceNo + 1;
  };

  export const isSequenceNoExistsInGoodReceivedNote = (grnId: string, sequenceNo: number): boolean => {
    return goodReceivedNoteDetails.some(detail => detail.good_received_note_id === grnId && detail.sequence_no === sequenceNo);
  };

  export const isGoodReceivedNoteSequenceNoExistsAll = (sequenceNo: number): boolean => {
    return goodReceivedNoteDetails.some(detail => detail.sequence_no === sequenceNo);
  };

  // Calculation functions
  export const calculateTotalAmount = (price: string, qty: string, taxAmount: string, discountAmount: string): string => {
  ...
  export const calculateBaseTotalAmount = (basePrice: string, baseQty: string): string => {

...
export const getStockInsByStatus = (
...
export const getStockInsByWorkflow = (workflowId: string): StockIn[] => {
  return stockIns.filter(
    (si) => si.workflow_id === workflowId && !si.deleted_at
  );
};

export const getStockInsByCreator = (createdById: string): StockIn[] => {
  return stockIns.filter(
    (si) => si.created_by_id === createdById && !si.deleted_at
  );
};

export const getStockInsByDateRange = (
  startDate: string,
  endDate: string
): StockIn[] => {
...
export const updateStockIn = (
  id: string,
  data: Partial<Omit<StockIn, "id" | "created_at" | "created_by_id">>
): StockIn | null => {
...
export const updateStockInStatus = (
  id: string,
  status: StockIn["doc_status"]
): StockIn | null => {
...
export const updateStockInWorkflow = (
  id: string,
  workflowId: string,
  workflowName: string
): StockIn | null => {
...
export const updateStockInDimension = (
  id: string,
  dimension: any
): StockIn | null => {
...
export const softDeleteStockIn = (
  id: string,
  deletedById: string
): StockIn | null => {
...
export const searchStockIns = (criteria: {
  ...
  return stockIns.filter((si) => {
    ...
    if (criteria.workflowId && si.workflow_id !== criteria.workflowId)
      return false;
    if (criteria.createdById && si.created_by_id !== criteria.createdById)
      return false;
    ...
    if (
      ...
      !si.description.toLowerCase().includes(criteria.description.toLowerCase())
    )
      return false;
    if (
      criteria.note &&
      !si.note.toLowerCase().includes(criteria.note.toLowerCase())
    )
      return false;
...
export const getStockInCountByStatus = (
  status: StockIn["doc_status"]
): number => {
  return stockIns.filter((si) => si.doc_status === status && !si.deleted_at)
    .length;
...
export const getNextStockInNo = (): string => {
  ...
  const yearStockIns = stockIns.filter(
    (si) => si.si_no.includes(`SI-${currentYear}-`) && !si.deleted_at
  );

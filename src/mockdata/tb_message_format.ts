...
export const messageFormats: MessageFormat[] = [
  {
    ...
    message:
      "Welcome to our system, {user_name}! We're glad to have you on board.",
    is_email: true,
    is_sms: false,
    is_in_app: true,
  ...
  },
  {
    ...
    message:
      "Your order {order_id} has been confirmed. Total amount: {total_amount}",
    is_email: true,
    is_sms: true,
    is_in_app: true,
    created_at: "2024-01-15T10:30:00Z",
...
export const createMessageFormat = (
  messageFormatData: Omit<MessageFormat, "id" | "created_at" | "updated_at">
): MessageFormat => {
...
export const getEmailAndSMSMessageFormats = (): MessageFormat[] => {
  return messageFormats.filter(
    (messageFormat) => messageFormat.is_email && messageFormat.is_sms
  );
};
...
export const getEmailAndInAppMessageFormats = (): MessageFormat[] => {
  return messageFormats.filter(
    (messageFormat) => messageFormat.is_email && messageFormat.is_in_app
  );
};
...
export const getSMSAndInAppMessageFormats = (): MessageFormat[] => {
  return messageFormats.filter(
    (messageFormat) => messageFormat.is_sms && messageFormat.is_in_app
  );
};
...
export const updateMessageFormat = (
  id: string,
  updateData: Partial<
    Omit<MessageFormat, "id" | "created_at" | "created_by_id">
  >
): MessageFormat | null => {
  const index = messageFormats.findIndex(
    (messageFormat) => messageFormat.id === id
  );
...
export const updateMessageFormatName = (
  id: string,
  name: string
): MessageFormat | null => {
...
export const updateMessageFormatMessage = (
  id: string,
  message: string
): MessageFormat | null => {
...
export const updateMessageFormatEmailChannel = (
  id: string,
  isEmail: boolean
): MessageFormat | null => {
...
export const updateMessageFormatSMSChannel = (
  id: string,
  isSMS: boolean
): MessageFormat | null => {
...
export const updateMessageFormatInAppChannel = (
  id: string,
  isInApp: boolean
): MessageFormat | null => {
...
export const updateMessageFormatChannels = (
  id: string,
  isEmail: boolean,
  isSMS: boolean,
  isInApp: boolean
...
export const deleteNonEmailMessageFormats = (): number => {
  ...
  const filteredMessageFormats = messageFormats.filter(
    (messageFormat) => messageFormat.is_email
  );
...
export const deleteNonSMSMessageFormats = (): number => {
  const initialLength = messageFormats.length;
  const filteredMessageFormats = messageFormats.filter(
    (messageFormat) => messageFormat.is_sms
  );
  const deletedCount = initialLength - filteredMessageFormats.length;
...
export const deleteNonInAppMessageFormats = (): number => {
  const initialLength = messageFormats.length;
  const filteredMessageFormats = messageFormats.filter(
    (messageFormat) => messageFormat.is_in_app
  );
  const deletedCount = initialLength - filteredMessageFormats.length;
...
export const searchMessageFormats = (searchCriteria: {
  ...
  return messageFormats.filter((messageFormat) => {
    ...
    if (
      searchCriteria.is_email !== undefined &&
      messageFormat.is_email !== searchCriteria.is_email
    ) {
      return false;
    }

    if (
      searchCriteria.is_sms !== undefined &&
      messageFormat.is_sms !== searchCriteria.is_sms
    ) {
      return false;
    }

    if (
      searchCriteria.is_in_app !== undefined &&
      messageFormat.is_in_app !== searchCriteria.is_in_app
    ) {
      return false;
    }
    ...
    if (searchCriteria.channel_type) {
      const channelCount = [
        messageFormat.is_email,
        messageFormat.is_sms,
        messageFormat.is_in_app,
      ].filter(Boolean).length;

      if (searchCriteria.channel_type === "email" && !messageFormat.is_email)
        return false;
      if (searchCriteria.channel_type === "sms" && !messageFormat.is_sms)
        return false;
      if (searchCriteria.channel_type === "in_app" && !messageFormat.is_in_app)
        return false;
      if (searchCriteria.channel_type === "multi" && channelCount < 2)
        return false;
    }

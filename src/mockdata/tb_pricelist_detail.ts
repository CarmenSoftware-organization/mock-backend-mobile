...
export const createPricelistDetail = (detailData: Omit<PricelistDetail, 'id' | 'created_at' | 'updated_at'>): PricelistDetail => {
...
export const getPricelistDetailById = (id: string): PricelistDetail | undefined => {
  return pricelistDetails.find(detail => detail.id === id);
};

// READ - อ่าน PricelistDetail ตาม pricelist_id
export const getPricelistDetailsByPricelistId = (pricelistId: string): PricelistDetail[] => {
  return pricelistDetails.filter(detail => detail.pricelist_id === pricelistId);
};

// READ - อ่าน PricelistDetail ตาม product_id
export const getPricelistDetailsByProductId = (productId: string): PricelistDetail[] => {
...
export const getPricelistDetailBySequence = (pricelistId: string, sequenceNo: number): PricelistDetail | undefined => {
  return pricelistDetails.find(detail => 
    detail.pricelist_id === pricelistId && detail.sequence_no === sequenceNo
  );
};

// READ - อ่าน PricelistDetail ตาม price range
export const getPricelistDetailsByPriceRange = (minPrice: number, maxPrice: number): PricelistDetail[] => {
...
export const searchPricelistDetails = (searchTerm: string): PricelistDetail[] => {
...
export const updatePricelistDetail = (id: string, updateData: Partial<Omit<PricelistDetail, 'id' | 'created_at' | 'created_by_id'>>): PricelistDetail | null => {
...
export const updatePricelistDetailPrice = (id: string, price: string): PricelistDetail | null => {
  return updatePricelistDetail(id, { price });
};

// UPDATE - อัปเดต PricelistDetail sequence
export const updatePricelistDetailSequence = (id: string, sequenceNo: number): PricelistDetail | null => {
  return updatePricelistDetail(id, { sequence_no: sequenceNo });
};

// UPDATE - อัปเดต PricelistDetail active status
export const updatePricelistDetailActiveStatus = (id: string, isActive: boolean): PricelistDetail | null => {
  return updatePricelistDetail(id, { is_active: isActive });
};

// DELETE - ลบ PricelistDetail (soft delete)
export const deletePricelistDetail = (id: string, deletedById: string): boolean => {
  const index = pricelistDetails.findIndex(detail => detail.id === id);
...
export const deletePricelistDetailsByPricelistId = (pricelistId: string, deletedById: string): number => {
  let deletedCount = 0;
...
export const deletePricelistDetailsByProductId = (productId: string, deletedById: string): number => {
  let deletedCount = 0;
...
export const getPricelistDetailCountByPricelistId = (pricelistId: string): number => {
  return pricelistDetails.filter(detail => detail.pricelist_id === pricelistId).length;
};
...
export const isSequenceNoExistsInPricelist = (pricelistId: string, sequenceNo: number): boolean => {
  return pricelistDetails.some(detail => 
    detail.pricelist_id === pricelistId && detail.sequence_no === sequenceNo
  );
};
...
export const searchPricelistDetailsAdvanced = (searchCriteria: {
  ...
  return pricelistDetails.filter(detail => {
    if (searchCriteria.pricelist_id && detail.pricelist_id !== searchCriteria.pricelist_id) {
      return false;
    }
    ...
    if (searchCriteria.price_min !== undefined || searchCriteria.price_max !== undefined) {
      const price = parseFloat(detail.price);
      if (searchCriteria.price_min !== undefined && price < searchCriteria.price_min) {
        return false;
      }
      if (searchCriteria.price_max !== undefined && price > searchCriteria.price_max) {
        return false;
      }
    }
    ...
    if (searchCriteria.tax_rate && detail.tax_rate !== searchCriteria.tax_rate) {
      return false;
    }
...
export const calculatePricelistTotalWithoutVat = (pricelistId: string): number => {
  return pricelistDetails
    .filter(detail => detail.pricelist_id === pricelistId && detail.is_active)

...
export const createUnitComment = (data: Omit<UnitComment, 'id' | 'created_at' | 'created_by_id' | 'updated_at' | 'updated_by_id'>): UnitComment => {
...
export const getUnitCommentsByType = (type: 'unit' | 'conversion' | 'other'): UnitComment[] => {
  return unitComments.filter(comment => comment.type === type && !comment.deleted_at);
};

export const getUnitCommentsByUser = (userId: string): UnitComment[] => {
  return unitComments.filter(comment => comment.user_id === userId && !comment.deleted_at);
};

export const getUnitCommentsByDateRange = (startDate: string, endDate: string): UnitComment[] => {
...
export const getUnitCommentsWithAttachments = (): UnitComment[] => {
  return unitComments.filter(comment => comment.attachments !== null && !comment.deleted_at);
};

export const getUnitCommentsWithNote = (): UnitComment[] => {
  return unitComments.filter(comment => comment.note !== null && !comment.deleted_at);
};

// UPDATE - อัปเดต UnitComment
export const updateUnitComment = (id: string, data: Partial<Omit<UnitComment, 'id' | 'created_at' | 'created_by_id'>>): UnitComment | null => {
  const index = unitComments.findIndex(comment => comment.id === id && !comment.deleted_at);
...
export const updateUnitCommentMessage = (id: string, message: string): UnitComment | null => {
  return updateUnitComment(id, { message });
};

// UPDATE - อัปเดต UnitComment note
export const updateUnitCommentNote = (id: string, note: string): UnitComment | null => {
  return updateUnitComment(id, { note });
};

// UPDATE - อัปเดต UnitComment info
export const updateUnitCommentInfo = (id: string, info: any): UnitComment | null => {
  return updateUnitComment(id, { info });
};

// UPDATE - อัปเดต UnitComment attachments
export const updateUnitCommentAttachments = (id: string, attachments: any): UnitComment | null => {
  return updateUnitComment(id, { attachments });
};

// DELETE - Soft delete UnitComment
export const softDeleteUnitComment = (id: string, deletedById: string): UnitComment | null => {
...
export const deleteUnitCommentsByUser = (userId: string, deletedById: string): boolean => {
...
export const searchUnitComments = (criteria: {
  ...
  return unitComments.filter(comment => {
    ...
    if (criteria.message && !comment.message.toLowerCase().includes(criteria.message.toLowerCase())) {
      return false;
    }
...
export const getUnitCommentCountByType = (type: 'unit' | 'conversion' | 'other'): number => {
  return unitComments.filter(comment => comment.type === type && !comment.deleted_at).length;
};

export const getUnitCommentCountByUser = (userId: string): number => {
  return unitComments.filter(comment => comment.user_id === userId && !comment.deleted_at).length;
};

export const isUnitCommentExists = (id: string): boolean => {
  return unitComments.some(comment => comment.id === id && !comment.deleted_at);
};

export const hasUnitCommentsByType = (type: 'unit' | 'conversion' | 'other'): boolean => {
  return unitComments.some(comment => comment.type === type && !comment.deleted_at);
};

export const hasUnitCommentsByUser = (userId: string): boolean => {
  return unitComments.some(comment => comment.user_id === userId && !comment.deleted_at);
};

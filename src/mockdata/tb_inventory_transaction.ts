...
export const getInventoryTransactionsByType = (
  transactionType: string
): InventoryTransaction[] => {
  return inventoryTransactions.filter(
    (transaction) => transaction.info?.transaction_type === transactionType
  );
};
...
export const getInventoryTransactionsByProduct = (
  ...
  return inventoryTransactions.filter(
    (transaction) => transaction.info?.product_id === productId
  );
};

// READ - อ่าน InventoryTransaction ตาม location_id
export const getInventoryTransactionsByLocation = (
  locationId: string
): InventoryTransaction[] => {
  return inventoryTransactions.filter(
    (transaction) => transaction.info?.location_id === locationId
  );
};

// READ - อ่าน InventoryTransaction ตาม transaction_date
export const getInventoryTransactionsByDate = (
  transactionDate: string
): InventoryTransaction[] => {
  return inventoryTransactions.filter(
    (transaction) => transaction.info?.transaction_date === transactionDate
  );
};

// READ - อ่าน InventoryTransaction ตาม date range
export const getInventoryTransactionsByDateRange = (
  startDate: string,
  endDate: string
): InventoryTransaction[] => {
  return inventoryTransactions.filter(
    (transaction) =>
      transaction.info?.transaction_date >= startDate &&
      transaction.info?.transaction_date <= endDate
  );
};

// READ - อ่าน InventoryTransaction ตาม reference_number
export const getInventoryTransactionByReference = (
  referenceNumber: string
): InventoryTransaction | undefined => {
  return inventoryTransactions.find(
    (transaction) => transaction.info?.reference_number === referenceNumber
  );
};

// READ - อ่าน InventoryTransaction ตาม currency_id
export const getInventoryTransactionsByCurrency = (
  currencyId: string
): InventoryTransaction[] => {
  return inventoryTransactions.filter(
    (transaction) => transaction.info?.currency_id === currencyId
  );
};
...
export const getInventoryTransactionsWithNotes = (): InventoryTransaction[] => {
  return inventoryTransactions.filter(
    (transaction) => transaction.note && transaction.note.trim() !== ""
  );
};

// READ - อ่าน InventoryTransaction ตาม quantity range
export const getInventoryTransactionsByQuantityRange = (
  minQuantity: number,
  maxQuantity: number
): InventoryTransaction[] => {
  return inventoryTransactions.filter(
    (transaction) =>
      transaction.info?.quantity >= minQuantity &&
      transaction.info?.quantity <= maxQuantity
  );
};

// READ - อ่าน InventoryTransaction ตาม unit price range
export const getInventoryTransactionsByPriceRange = (
  minPrice: number,
  maxPrice: number
): InventoryTransaction[] => {
  return inventoryTransactions.filter(
    (transaction) =>
      transaction.info?.unit_price >= minPrice &&
      transaction.info?.unit_price <= maxPrice
  );
};

// READ - ค้นหา InventoryTransaction แบบ fuzzy search
export const searchInventoryTransactions = (
  searchTerm: string
): InventoryTransaction[] => {
  const lowerSearchTerm = searchTerm.toLowerCase();
  return inventoryTransactions.filter(
    (transaction) =>
      transaction.info?.reference_number
        ?.toLowerCase()
        .includes(lowerSearchTerm) ||
      (transaction.note &&
        transaction.note.toLowerCase().includes(lowerSearchTerm))
  );
};
...
export const updateInventoryTransaction = (
  id: string,
  updateData: Partial<
    Omit<InventoryTransaction, "id" | "created_at" | "created_by_id">
  >
): InventoryTransaction | null => {
  const index = inventoryTransactions.findIndex(
...
export const updateInventoryTransactionType = (
  id: string,
  transactionType: string
): InventoryTransaction | null => {
  return updateInventoryTransaction(id, {
    info: {
      ...inventoryTransactions.find((t) => t.id === id)?.info,
...
export const updateInventoryTransactionQuantity = (
  id: string,
  quantity: number
): InventoryTransaction | null => {
  return updateInventoryTransaction(id, {
    info: { ...inventoryTransactions.find((t) => t.id === id)?.info, quantity },
  });
};

// UPDATE - อัปเดต InventoryTransaction unit price
export const updateInventoryTransactionUnitPrice = (
  id: string,
  unitPrice: number
): InventoryTransaction | null => {
  return updateInventoryTransaction(id, {
...
export const updateInventoryTransactionDate = (
  id: string,
  transactionDate: string
): InventoryTransaction | null => {
  return updateInventoryTransaction(id, {
...
export const updateInventoryTransactionReference = (
  id: string,
  referenceNumber: string
): InventoryTransaction | null => {
  return updateInventoryTransaction(id, {
...
export const updateInventoryTransactionNotes = (
  id: string,
  notes: string
): InventoryTransaction | null => {
...
export const updateInventoryTransactionProduct = (
  id: string,
  productId: string
): InventoryTransaction | null => {
  return updateInventoryTransaction(id, {
...
export const updateInventoryTransactionLocation = (
  id: string,
  locationId: string
): InventoryTransaction | null => {
  return updateInventoryTransaction(id, {
...
export const updateInventoryTransactionCurrency = (
  id: string,
  currencyId: string
): InventoryTransaction | null => {
  return updateInventoryTransaction(id, {
...
export const deleteInventoryTransactionByReference = (
  referenceNumber: string,
  deletedById: string
): boolean => {
...
export const deleteInventoryTransactionsByType = (
  transactionType: string,
  deletedById: string
): number => {
  let deletedCount = 0;
  ...
  inventoryTransactions.forEach((transaction) => {
    if (
      transaction.info?.transaction_type === transactionType &&
...
export const deleteInventoryTransactionsByProduct = (
  productId: string,
  deletedById: string
): number => {
  let deletedCount = 0;
...
export const deleteInventoryTransactionsByLocation = (
  locationId: string,
  deletedById: string
): number => {
  let deletedCount = 0;
  ...
  inventoryTransactions.forEach((transaction) => {
    if (
      transaction.info?.location_id === locationId &&
      !transaction.deleted_at
    ) {
      transaction.deleted_at = getCurrentTimestamp();
      transaction.deleted_by_id = deletedById;
...
export const getInventoryTransactionCountByType = (
  transactionType: string
): number => {
  return inventoryTransactions.filter(
    (transaction) => transaction.info?.transaction_type === transactionType
...
export const getInventoryTransactionCountByProduct = (
  productId: string
): number => {
  return inventoryTransactions.filter(
    (transaction) => transaction.info?.product_id === productId
...
export const getInventoryTransactionCountByLocation = (
  locationId: string
): number => {
  return inventoryTransactions.filter(
    (transaction) => transaction.info?.location_id === locationId
...
export const getInventoryTransactionCountByCurrency = (
  currencyId: string
): number => {
  return inventoryTransactions.filter(
    (transaction) => transaction.info?.currency_id === currencyId
...
export const isInventoryTransactionReferenceExists = (
  referenceNumber: string
): boolean => {
...
export const searchInventoryTransactionsAdvanced = (searchCriteria: {
  ...
  return inventoryTransactions.filter((transaction) => {
    if (
      searchCriteria.transaction_type &&
      transaction.info?.transaction_type !== searchCriteria.transaction_type
    ) {
      return false;
    }
    ...
    if (
      ...
      transaction.info?.product_id !== searchCriteria.product_id
    ) {
      return false;
    }
    ...
    if (
      ...
      transaction.info?.location_id !== searchCriteria.location_id
    ) {
      return false;
    }
    ...
    if (
      ...
      transaction.info?.transaction_date !== searchCriteria.transaction_date
    ) {
      return false;
    }

    if (
      searchCriteria.reference_number &&
      !transaction.info?.reference_number
        ?.toLowerCase()
        .includes(searchCriteria.reference_number.toLowerCase())
    ) {
      return false;
    }
    ...
    if (
      ...
      transaction.info?.currency_id !== searchCriteria.currency_id
    ) {
      return false;
    }

    if (
      searchCriteria.min_quantity &&
      transaction.info?.quantity < searchCriteria.min_quantity
    ) {
      return false;
    }

    if (
      searchCriteria.max_quantity &&
      transaction.info?.quantity > searchCriteria.max_quantity
    ) {
      return false;
    }
    ...
    if (
      ...
      transaction.info?.unit_price < searchCriteria.min_price
    ) {
      return false;
    }
    ...
    if (
      ...
      transaction.info?.unit_price > searchCriteria.max_price
    ) {
      return false;
    }

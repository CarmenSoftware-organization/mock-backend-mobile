...
export const createVendorContact = (data: Omit<VendorContact, 'id' | 'created_at' | 'created_by_id' | 'updated_at' | 'updated_by_id'>): VendorContact => {
...
export const getVendorContactsByVendor = (vendorId: string): VendorContact[] => {
  return vendorContacts.filter(contact => contact.vendor_id === vendorId && !contact.deleted_at);
};

export const getVendorContactsByType = (contactType: 'phone' | 'email' | 'fax' | 'website' | 'other'): VendorContact[] => {
  return vendorContacts.filter(contact => contact.contact_type === contactType && !contact.deleted_at);
};

export const getVendorContactsByVendorAndType = (vendorId: string, contactType: 'phone' | 'email' | 'fax' | 'website' | 'other'): VendorContact[] => {
...
export const getActiveVendorContacts = (): VendorContact[] => {
  return vendorContacts.filter(contact => contact.is_active && !contact.deleted_at);
};
...
export const getVendorContactsByCreator = (createdById: string): VendorContact[] => {
  return vendorContacts.filter(contact => contact.created_by_id === createdById && !contact.deleted_at);
};

export const getVendorContactsByDateRange = (startDate: string, endDate: string): VendorContact[] => {
  return vendorContacts.filter(contact => {
...
export const getVendorContactsWithNote = (): VendorContact[] => {
  return vendorContacts.filter(contact => contact.note !== null && !contact.deleted_at);
};
...
export const updateVendorContact = (id: string, data: Partial<Omit<VendorContact, 'id' | 'created_at' | 'created_by_id'>>): VendorContact | null => {
  const index = vendorContacts.findIndex(contact => contact.id === id && !contact.deleted_at);
...
export const updateVendorContactStatus = (id: string, isActive: boolean): VendorContact | null => {
  return updateVendorContact(id, { is_active: isActive });
};

// UPDATE - อัปเดต VendorContact type
export const updateVendorContactType = (id: string, contactType: 'phone' | 'email' | 'fax' | 'website' | 'other'): VendorContact | null => {
  return updateVendorContact(id, { contact_type: contactType });
};

// UPDATE - อัปเดต VendorContact description
export const updateVendorContactDescription = (id: string, description: string): VendorContact | null => {
  return updateVendorContact(id, { description });
};

// UPDATE - อัปเดต VendorContact note
export const updateVendorContactNote = (id: string, note: string): VendorContact | null => {
  return updateVendorContact(id, { note });
};

// UPDATE - อัปเดต VendorContact info
export const updateVendorContactInfo = (id: string, info: any): VendorContact | null => {
  return updateVendorContact(id, { info });
};

// UPDATE - อัปเดต VendorContact dimension
export const updateVendorContactDimension = (id: string, dimension: any): VendorContact | null => {
  return updateVendorContact(id, { dimension });
};

// DELETE - Soft delete VendorContact
export const softDeleteVendorContact = (id: string, deletedById: string): VendorContact | null => {
...
export const deleteVendorContactsByVendor = (vendorId: string, deletedById: string): boolean => {
...
export const deleteVendorContactsByType = (contactType: 'phone' | 'email' | 'fax' | 'website' | 'other', deletedById: string): boolean => {
...
export const searchVendorContacts = (criteria: {
  ...
  return vendorContacts.filter(contact => {
    ...
    if (criteria.contact_type && contact.contact_type !== criteria.contact_type) {
      return false;
    }
    
    if (criteria.is_active !== undefined && contact.is_active !== criteria.is_active) {
      return false;
    }
    ...
    if (criteria.created_by_id && contact.created_by_id !== criteria.created_by_id) {
      return false;
    }
...
export const getVendorContactCountByVendor = (vendorId: string): number => {
  return vendorContacts.filter(contact => contact.vendor_id === vendorId && !contact.deleted_at).length;
};

export const getVendorContactCountByType = (contactType: 'phone' | 'email' | 'fax' | 'website' | 'other'): number => {
  return vendorContacts.filter(contact => contact.contact_type === contactType && !contact.deleted_at).length;
};

export const getActiveVendorContactCount = (): number => {
  return vendorContacts.filter(contact => contact.is_active && !contact.deleted_at).length;
};

export const isVendorContactExists = (id: string): boolean => {
  return vendorContacts.some(contact => contact.id === id && !contact.deleted_at);
};

export const hasVendorContactsByVendor = (vendorId: string): boolean => {
  return vendorContacts.some(contact => contact.vendor_id === vendorId && !contact.deleted_at);
};

export const hasVendorContactsByType = (contactType: 'phone' | 'email' | 'fax' | 'website' | 'other'): boolean => {
  return vendorContacts.some(contact => contact.contact_type === contactType && !contact.deleted_at);
};

export const hasVendorContactsWithNote = (): boolean => {
  return vendorContacts.some(contact => contact.note !== null && !contact.deleted_at);
};

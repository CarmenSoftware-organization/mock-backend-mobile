...
export const createDeliveryPoint = (
  deliveryPointData: Omit<DeliveryPoint, "id" | "created_at" | "updated_at">
): DeliveryPoint => {
...
export const getDeliveryPointByName = (
  name: string
...
export const getDeliveryPointsByContactPerson = (
  contactPerson: string
): DeliveryPoint[] => {
  return deliveryPoints.filter((deliveryPoint) =>
    deliveryPoint.info?.contact_person
      ?.toLowerCase()
...
export const getDeliveryPointByEmail = (
  email: string
): DeliveryPoint | undefined => {
  return deliveryPoints.find(
    (deliveryPoint) => deliveryPoint.info?.email === email
  );
};

// READ - อ่าน DeliveryPoint ตาม phone
export const getDeliveryPointByPhone = (
  phone: string
): DeliveryPoint | undefined => {
  return deliveryPoints.find(
    (deliveryPoint) => deliveryPoint.info?.phone === phone
  );
};

// UPDATE - อัปเดต DeliveryPoint
export const updateDeliveryPoint = (
  id: string,
  updateData: Partial<
    Omit<DeliveryPoint, "id" | "created_at" | "created_by_id">
  >
): DeliveryPoint | null => {
  const index = deliveryPoints.findIndex(
...
export const updateDeliveryPointName = (
  id: string,
  name: string
): DeliveryPoint | null => {
...
export const updateDeliveryPointAddress = (
  id: string,
  address: string
): DeliveryPoint | null => {
...
export const updateDeliveryPointContactPerson = (
  id: string,
  contactPerson: string
): DeliveryPoint | null => {
  const deliveryPoint = getDeliveryPointById(id);
...
export const updateDeliveryPointPhone = (
  id: string,
  phone: string
): DeliveryPoint | null => {
  const deliveryPoint = getDeliveryPointById(id);
...
export const updateDeliveryPointEmail = (
  id: string,
  email: string
): DeliveryPoint | null => {
  const deliveryPoint = getDeliveryPointById(id);
...
export const updateDeliveryPointActiveStatus = (
  id: string,
  isActive: boolean
): DeliveryPoint | null => {
...
export const deleteDeliveryPoint = (
  id: string,
  deletedById: string
): boolean => {
  const index = deliveryPoints.findIndex(
...
export const deleteDeliveryPointByName = (
  name: string,
  deletedById: string
): boolean => {
...
export const deleteDeliveryPointsByCity = (
  city: string,
  deletedById: string
): number => {
  let deletedCount = 0;
...
export const searchDeliveryPoints = (searchCriteria: {
  ...
  return deliveryPoints.filter((deliveryPoint) => {
    ...
    if (
      searchCriteria.contact_person &&
      !deliveryPoint.info?.contact_person
        ?.toLowerCase()
        .includes(searchCriteria.contact_person.toLowerCase())
    ) {
      return false;
    }
    ...
    if (
      ...
      deliveryPoint.info?.email !== searchCriteria.email
    ) {
      return false;
    }
    ...
    if (
      ...
      deliveryPoint.info?.phone !== searchCriteria.phone
    ) {
      return false;
    }
    ...
    if (
      ...
      deliveryPoint.is_active !== searchCriteria.is_active
    ) {
      return false;
    }
...
export const getDeletedDeliveryPoints = (): DeliveryPoint[] => {
  return deliveryPoints.filter(
    (deliveryPoint) => deliveryPoint.deleted_at !== null
  );
};

...
export const createCurrency = (currencyData: Omit<Currency, 'id' | 'created_at' | 'updated_at'>): Currency => {
...
export const getCurrenciesByDecimalPlaces = (decimalPlaces: number): Currency[] => {
  return currencies.filter(currency => currency.decimal_places === decimalPlaces);
};
...
export const updateCurrency = (id: string, updateData: Partial<Omit<Currency, 'id' | 'created_at' | 'created_by_id'>>): Currency | null => {
...
export const updateCurrencyStatus = (id: string, isActive: boolean): Currency | null => {
  return updateCurrency(id, { is_active: isActive });
};

// UPDATE - อัปเดต Currency exchange rate
export const updateCurrencyExchangeRate = (id: string, exchangeRate: number): Currency | null => {
...
export const updateCurrencyDecimalPlaces = (id: string, decimalPlaces: number): Currency | null => {
  return updateCurrency(id, { decimal_places: decimalPlaces });
};

// UPDATE - อัปเดต Currency symbol
export const updateCurrencySymbol = (id: string, symbol: string): Currency | null => {
  return updateCurrency(id, { symbol });
};

// UPDATE - อัปเดต Currency description
export const updateCurrencyDescription = (id: string, description: string): Currency | null => {
  return updateCurrency(id, { description });
};

// UPDATE - อัปเดต Currency note
export const updateCurrencyNote = (id: string, note: string): Currency | null => {
...
export const deleteCurrencyByCode = (code: string, deletedById: string): boolean => {
...
export const searchCurrencies = (searchCriteria: {
  ...
  return currencies.filter(currency => {
    if (searchCriteria.code && !currency.code.toLowerCase().includes(searchCriteria.code.toLowerCase())) {
      return false;
    }
    
    if (searchCriteria.name && !currency.name.toLowerCase().includes(searchCriteria.name.toLowerCase())) {
      return false;
    }
    ...
    if (searchCriteria.is_active !== undefined && currency.is_active !== searchCriteria.is_active) {
      return false;
    }
    
    if (searchCriteria.decimal_places !== undefined && currency.decimal_places !== searchCriteria.decimal_places) {
      return false;
    }

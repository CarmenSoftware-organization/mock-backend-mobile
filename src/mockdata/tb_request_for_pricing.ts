...
export const createRequestForPricing = (
  data: Omit<RequestForPricing, "id" | "created_at" | "created_by_id">
): RequestForPricing => {
...
export const getRequestForPricingById = (
  id: string
): RequestForPricing | null => {
  const rfp = requestForPricings.find(
    (rfp) => rfp.id === id && !rfp.deleted_at
  );
  return rfp || null;
};

// READ - อ่าน RequestForPricing ตาม pricelist_template_id
export const getRequestForPricingsByPricelistTemplateId = (
  pricelistTemplateId: string
): RequestForPricing[] => {
  return requestForPricings.filter(
    (rfp) =>
      rfp.pricelist_template_id === pricelistTemplateId && !rfp.deleted_at
  );
};

// READ - อ่าน RequestForPricing ตาม start_date
export const getRequestForPricingsByStartDate = (
  startDate: string
): RequestForPricing[] => {
  return requestForPricings.filter(
    (rfp) => rfp.start_date === startDate && !rfp.deleted_at
  );
};

// READ - อ่าน RequestForPricings ตามช่วงวันที่
export const getRequestForPricingsByDateRange = (
  startDate: string,
  endDate: string
): RequestForPricing[] => {
...
export const searchRequestForPricings = (
  searchTerm: string
): RequestForPricing[] => {
...
export const updateRequestForPricing = (
  id: string,
  updates: Partial<
    Omit<RequestForPricing, "id" | "created_at" | "created_by_id">
  >
): RequestForPricing | null => {
...
export const updateRequestForPricingDates = (
  id: string,
  startDate: string,
  endDate: string
): RequestForPricing | null => {
...
export const updateRequestForPricingInfo = (
  id: string,
  info: any
): RequestForPricing | null => {
...
export const updateRequestForPricingDimension = (
  id: string,
  dimension: any
): RequestForPricing | null => {
...
export const updateRequestForPricingDocVersion = (
  id: string,
  docVersion: string
): RequestForPricing | null => {
...
export const deleteRequestForPricing = (
  id: string,
  deletedById: string
): boolean => {
  const index = requestForPricings.findIndex((rfp) => rfp.id === id);
...
export const deleteRequestForPricingsByPricelistTemplateId = (
  pricelistTemplateId: string,
  deletedById: string
): number => {
  let deletedCount = 0;
...
export const deleteExpiredRequestForPricings = (
  deletedById: string
): number => {
  let deletedCount = 0;
  const now = new Date();
...
export const restoreRequestForPricingsByPricelistTemplateId = (
  pricelistTemplateId: string
): number => {
  let restoredCount = 0;
...
export const getRequestForPricingCountByPricelistTemplateId = (
  pricelistTemplateId: string
): number => {
  return requestForPricings.filter(
    (rfp) => rfp.pricelist_template_id === pricelistTemplateId
  ).length;
};
...
export const hasRequestForPricingCreatedBy = (id: string): boolean => {
  const rfp = getRequestForPricingById(id);
  return rfp
    ? !!(rfp.created_by_id && (rfp.created_by_id as string).trim() !== "")
    : false;
};
...
export const hasRequestForPricingUpdatedBy = (id: string): boolean => {
  const rfp = getRequestForPricingById(id);
  return rfp
    ? !!(rfp.updated_by_id && (rfp.updated_by_id as string).trim() !== "")
    : false;
};
...
export const hasRequestForPricingDeletedBy = (id: string): boolean => {
  const rfp = getRequestForPricingById(id);
  return rfp
    ? !!(rfp.deleted_by_id && (rfp.deleted_by_id as string).trim() !== "")
    : false;
};
...
export const hasRequestForPricingCreatedAt = (id: string): boolean => {
  const rfp = getRequestForPricingById(id);
  return rfp
    ? !!(rfp.created_at && (rfp.created_at as string).trim() !== "")
    : false;
};
...
export const hasRequestForPricingUpdatedAt = (id: string): boolean => {
  const rfp = getRequestForPricingById(id);
  return rfp
    ? !!(rfp.updated_at && (rfp.updated_at as string).trim() !== "")
    : false;
};
...
export const hasRequestForPricingDeletedAt = (id: string): boolean => {
  const rfp = getRequestForPricingById(id);
  return rfp
    ? !!(rfp.deleted_at && (rfp.deleted_at as string).trim() !== "")
    : false;
};

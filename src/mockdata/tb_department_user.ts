...
export const departmentUsers: DepartmentUser[] = [
  {
    id: "58ef304a-659a-446f-868c-419fb1fd4eaf",
  ...
  {
    id: "4bc07e43-8b0d-4050-bdf9-6e15fdd23917",
  ...
  {
    id: "8f850271-b459-43c1-93a1-28aadad2745d",
  ...
  {
    id: "08be6ae7-9987-413c-8d2a-d507fe86a457",
  ...
  {
    id: "c316abf2-ed7d-4998-9e2f-985cb4c96911",
    user_id: "57c0721a-4afd-4e8f-b118-f0480c537215",
    department_id: "114512b8-5983-4ae0-b9c5-0fb4de8b1059",
    is_hod: false,
    note: null,
    info: null,
    dimension: null,
  ...
  {
    id: "90f1f16f-90e8-4c14-9b67-c307dc2ff5ab",
    user_id: "59c4ce87-84e5-48e5-b246-7d0f6d2c5594",
    department_id: "6a6e0e3c-d09c-46f6-9609-9ad0c35af09b",
    is_hod: false,
    note: null,
    info: null,
    dimension: null,
...
export const createDepartmentUser = (departmentUserData: Omit<DepartmentUser, 'id' | 'created_at' | 'updated_at'>): DepartmentUser => {
...
export const getDepartmentUserById = (id: string): DepartmentUser | undefined => {
  return departmentUsers.find(departmentUser => departmentUser.id === id);
};

// READ - อ่าน DepartmentUser ตาม user_id
export const getDepartmentUsersByUserId = (userId: string): DepartmentUser[] => {
  return departmentUsers.filter(departmentUser => departmentUser.user_id === userId);
};

// READ - อ่าน DepartmentUser ตาม department_id
export const getDepartmentUsersByDepartmentId = (departmentId: string): DepartmentUser[] => {
  return departmentUsers.filter(departmentUser => departmentUser.department_id === departmentId);
};

// READ - อ่าน DepartmentUser ที่เป็น HOD
export const getDepartmentUsersByHodStatus = (isHod: boolean): DepartmentUser[] => {
  return departmentUsers.filter(departmentUser => departmentUser.is_hod === isHod);
};

// READ - อ่าน DepartmentUser ที่ active (ไม่ถูกลบ)
export const getActiveDepartmentUsers = (): DepartmentUser[] => {
  return departmentUsers.filter(departmentUser => departmentUser.deleted_at === null);
};
...
export const getDepartmentUsersByDocVersion = (docVersion: string): DepartmentUser[] => {
  return departmentUsers.filter(departmentUser => departmentUser.doc_version === docVersion);
};

// READ - อ่าน DepartmentUser ตาม created_by_id
export const getDepartmentUsersByCreatedBy = (createdById: string): DepartmentUser[] => {
  return departmentUsers.filter(departmentUser => departmentUser.created_by_id === createdById);
};

// READ - อ่าน DepartmentUser ตาม updated_by_id
export const getDepartmentUsersByUpdatedBy = (updatedById: string): DepartmentUser[] => {
  return departmentUsers.filter(departmentUser => departmentUser.updated_by_id === updatedById);
};

// UPDATE - อัปเดต DepartmentUser
export const updateDepartmentUser = (id: string, updateData: Partial<Omit<DepartmentUser, 'id' | 'created_at' | 'created_by_id'>>): DepartmentUser | null => {
  const index = departmentUsers.findIndex(departmentUser => departmentUser.id === id);
...
export const updateDepartmentUserHodStatus = (id: string, isHod: boolean): DepartmentUser | null => {
  return updateDepartmentUser(id, { is_hod: isHod });
};

// UPDATE - อัปเดต DepartmentUser note
export const updateDepartmentUserNote = (id: string, note: string): DepartmentUser | null => {
  return updateDepartmentUser(id, { note });
};

// UPDATE - DepartmentUser info
export const updateDepartmentUserInfo = (id: string, info: any): DepartmentUser | null => {
  return updateDepartmentUser(id, { info });
};

// UPDATE - DepartmentUser dimension
export const updateDepartmentUserDimension = (id: string, dimension: any): DepartmentUser | null => {
  return updateDepartmentUser(id, { dimension });
};

// UPDATE - DepartmentUser doc_version
export const updateDepartmentUserDocVersion = (id: string, docVersion: string): DepartmentUser | null => {
  return updateDepartmentUser(id, { doc_version: docVersion });
};

// UPDATE - DepartmentUser โดย user_id และ department_id
export const updateDepartmentUserByUserAndDepartment = (userId: string, departmentId: string, updateData: Partial<Omit<DepartmentUser, 'id' | 'created_at' | 'created_by_id'>>): DepartmentUser | null => {
  const index = departmentUsers.findIndex(du => du.user_id === userId && du.department_id === departmentId);
...
export const deleteDepartmentUser = (id: string, deletedById: string): boolean => {
  const index = departmentUsers.findIndex(departmentUser => departmentUser.id === id);
...
export const deleteDepartmentUsersByUserId = (userId: string, deletedById: string): number => {
  let deletedCount = 0;
...
export const deleteDepartmentUsersByDepartmentId = (departmentId: string, deletedById: string): number => {
  let deletedCount = 0;
  
  departmentUsers.forEach(departmentUser => {
    if (departmentUser.department_id === departmentId && !departmentUser.deleted_at) {
      departmentUser.deleted_at = getCurrentTimestamp();
      departmentUser.deleted_by_id = deletedById;
      deletedCount++;
    }
...
export const deleteDepartmentUserByUserAndDepartment = (userId: string, departmentId: string, deletedById: string): boolean => {
  const index = departmentUsers.findIndex(du => du.user_id === userId && du.department_id === departmentId);
...
export const restoreDepartmentUsersByDepartmentId = (departmentId: string): number => {
  let restoredCount = 0;
...
export const getActiveDepartmentUserCount = (): number => {
  return departmentUsers.filter(departmentUser => departmentUser.deleted_at === null).length;
};
...
export const getDepartmentUserCountByUserId = (userId: string): number => {
  return departmentUsers.filter(departmentUser => departmentUser.user_id === userId).length;
};

// Utility function สำหรับนับจำนวน DepartmentUser ตาม department_id
export const getDepartmentUserCountByDepartmentId = (departmentId: string): number => {
  return departmentUsers.filter(departmentUser => departmentUser.department_id === departmentId).length;
};

// Utility function สำหรับนับจำนวน DepartmentUser ที่เป็น HOD
export const getHodDepartmentUserCount = (): number => {
  return departmentUsers.filter(departmentUser => departmentUser.is_hod).length;
};
...
export const searchDepartmentUsers = (searchCriteria: {
  ...
  return departmentUsers.filter(departmentUser => {
    if (searchCriteria.user_id && departmentUser.user_id !== searchCriteria.user_id) {
      return false;
    }
    
    if (searchCriteria.department_id && departmentUser.department_id !== searchCriteria.department_id) {
      return false;
    }
    
    if (searchCriteria.is_hod !== undefined && departmentUser.is_hod !== searchCriteria.is_hod) {
      return false;
    }
    
    if (searchCriteria.doc_version && departmentUser.doc_version !== searchCriteria.doc_version) {
      return false;
    }
    
    if (searchCriteria.created_by_id && departmentUser.created_by_id !== searchCriteria.created_by_id) {
      return false;
    }
    
    if (searchCriteria.updated_by_id && departmentUser.updated_by_id !== searchCriteria.updated_by_id) {
      return false;
    }
...
export const isDepartmentUserExistsByUserId = (userId: string): boolean => {
  return departmentUsers.some(departmentUser => departmentUser.user_id === userId);
};

// Utility function สำหรับตรวจสอบ DepartmentUser ตาม department_id
export const isDepartmentUserExistsByDepartmentId = (departmentId: string): boolean => {
  return departmentUsers.some(departmentUser => departmentUser.department_id === departmentId);
};

// Utility function สำหรับตรวจสอบ DepartmentUser โดย user_id และ department_id
export const isDepartmentUserExistsByUserAndDepartment = (userId: string, departmentId: string): boolean => {
  return departmentUsers.some(du => du.user_id === userId && du.department_id === departmentId);
};
...
export const hasDepartmentUsersWithDimension = (): boolean => {
  return departmentUsers.some(departmentUser => departmentUser.dimension !== null);
};

// Bulk operations
// เพิ่ม DepartmentUser หลายรายการ
export const addMultipleDepartmentUsers = (departmentUsersData: Omit<DepartmentUser, 'id' | 'created_at' | 'updated_at'>[]): DepartmentUser[] => {
...
export const removeMultipleDepartmentUsersByUserId = (userId: string, deletedById: string): number => {
  return deleteDepartmentUsersByUserId(userId, deletedById);
};

// ลบ DepartmentUser หลายรายการตาม department_id
export const removeMultipleDepartmentUsersByDepartmentId = (departmentId: string, deletedById: string): number => {
...
export const hardDeleteDepartmentUsersByDepartmentId = (departmentId: string): number => {
  let deletedCount = 0;

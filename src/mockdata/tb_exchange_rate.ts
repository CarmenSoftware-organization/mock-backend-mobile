...
export const createExchangeRate = (
  exchangeRateData: Omit<ExchangeRate, "id" | "created_at" | "updated_at">
): ExchangeRate => {
...
export const getExchangeRatesByFromCurrency = (
  fromCurrencyId: string
): ExchangeRate[] => {
  return exchangeRates.filter(
    (exchangeRate) => exchangeRate.info.from_currency_id === fromCurrencyId
  );
};

// READ - อ่าน ExchangeRate ตาม to_currency_id
export const getExchangeRatesByToCurrency = (
  toCurrencyId: string
): ExchangeRate[] => {
  return exchangeRates.filter(
    (exchangeRate) => exchangeRate.info.to_currency_id === toCurrencyId
  );
};

// READ - อ่าน ExchangeRate ตาม currency pair
export const getExchangeRateByCurrencyPair = (
  fromCurrencyId: string,
  toCurrencyId: string
...
export const getActiveExchangeRates = (): ExchangeRate[] => {
  return exchangeRates.filter(
    (exchangeRate) => exchangeRate.info.is_active === true
  );
};
...
export const getInactiveExchangeRates = (): ExchangeRate[] => {
  return exchangeRates.filter(
    (exchangeRate) => exchangeRate.info.is_active === false
  );
};

// READ - อ่าน ExchangeRate ตาม effective_date
export const getExchangeRatesByEffectiveDate = (
  effectiveDate: string
): ExchangeRate[] => {
  return exchangeRates.filter(
    (exchangeRate) => exchangeRate.info.effective_date === effectiveDate
  );
};
...
export const getExpiredExchangeRates = (): ExchangeRate[] => {
  const today = new Date().toISOString().split("T")[0];
  return exchangeRates.filter(
    (exchangeRate) => exchangeRate.info.expiry_date < today
  );
};
...
export const updateExchangeRate = (
  id: string,
  updateData: Partial<Omit<ExchangeRate, "id" | "created_at" | "created_by_id">>
): ExchangeRate | null => {
...
export const updateExchangeRateRate = (
  id: string,
  rate: number
): ExchangeRate | null => {
...
export const updateExchangeRateEffectiveDate = (
  id: string,
  effectiveDate: string
): ExchangeRate | null => {
  return updateExchangeRate(id, {
    info: { ...exchangeRates[0].info, effective_date: effectiveDate },
  });
};

// UPDATE - อัปเดต ExchangeRate expiry_date
export const updateExchangeRateExpiryDate = (
  id: string,
  expiryDate: string
): ExchangeRate | null => {
  return updateExchangeRate(id, {
    info: { ...exchangeRates[0].info, expiry_date: expiryDate },
  });
};

// UPDATE - อัปเดต ExchangeRate active status
export const updateExchangeRateActiveStatus = (
  id: string,
  isActive: boolean
): ExchangeRate | null => {
  return updateExchangeRate(id, {
    info: { ...exchangeRates[0].info, is_active: isActive },
  });
};
...
export const deleteExchangeRateByCurrencyPair = (
  fromCurrencyId: string,
  toCurrencyId: string,
  deletedById: string
): boolean => {
  const exchangeRate = getExchangeRateByCurrencyPair(
    fromCurrencyId,
    toCurrencyId
  );
  if (exchangeRate) {
...
export const deleteExchangeRatesByFromCurrency = (
  fromCurrencyId: string,
  deletedById: string
): number => {
  let deletedCount = 0;
...
export const deleteExchangeRatesByToCurrency = (
  toCurrencyId: string,
  deletedById: string
): number => {
  let deletedCount = 0;
...
export const getExchangeRateCountByFromCurrency = (
  fromCurrencyId: string
): number => {
  return exchangeRates.filter(
    (exchangeRate) => exchangeRate.info.from_currency_id === fromCurrencyId
...
export const getExchangeRateCountByToCurrency = (
  toCurrencyId: string
): number => {
  return exchangeRates.filter(
    (exchangeRate) => exchangeRate.info.to_currency_id === toCurrencyId
...
export const isExchangeRateCurrencyPairExists = (
  fromCurrencyId: string,
  toCurrencyId: string
): boolean => {
...
export const getDeletedExchangeRates = (): ExchangeRate[] => {
  return exchangeRates.filter(
    (exchangeRate) => exchangeRate.deleted_at !== null
  );
};
...
export const searchExchangeRates = (searchCriteria: {
  ...
  return exchangeRates.filter((exchangeRate) => {
    if (
      searchCriteria.from_currency_id &&
      exchangeRate.info.from_currency_id !== searchCriteria.from_currency_id
    ) {
      return false;
    }

    if (
      searchCriteria.to_currency_id &&
      exchangeRate.info.to_currency_id !== searchCriteria.to_currency_id
    ) {
      return false;
    }

    if (
      searchCriteria.exchange_rate &&
      exchangeRate.exchange_rate !== searchCriteria.exchange_rate
    ) {
      return false;
    }

    if (
      searchCriteria.effective_date &&
      exchangeRate.info.effective_date !== searchCriteria.effective_date
    ) {
      return false;
    }

    if (
      searchCriteria.expiry_date &&
      exchangeRate.info.expiry_date !== searchCriteria.expiry_date
    ) {
      return false;
    }
    ...
    if (
      ...
      exchangeRate.info.is_active !== searchCriteria.is_active
    ) {
      return false;
    }
